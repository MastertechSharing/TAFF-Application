...
<% 	
	...
	session.setAttribute("action", "data_workcode.jsp?action="+getaction+"&");
...
<html lang="en">
	<head>
		...
		<script language="javascript">
			...
			function Confirm_Button(){
				location.href = 'module/act_workcode.jsp?action=del&work_code='+document.getElementById('sCode').value;
			}
	...
	<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onLoad="Clear_Session_Table('<%= clear_session %>', 'DataTables_data_table_<%= request.getRequestURI() %>'); Load_DataTable();">
				...
				<div class="table-responsive" style="border: 0px !important;" border="0">
					...
					<div style="min-width: 800px;" class="table" border="0">	
						...
						<table style="min-width: 800px;" align="center" id="data_table" class="table table-striped table-bordered" cellspacing="0" >
							...
							<%	if(checkPermission(ses_per, "01")){	%>	
									...
									<td width="3%" align="center"> </td>
							<%	}	%>
									<td width="20%" align="center"> <%= lb_work_code %> </td>
									<td width="30%" align="center"> <%= lb_thdesc %> </td>
									<td width="30%" align="center"> <%= lb_endesc %> </td>
									<td width="14%" align="center"> <%= lb_mapoutput %> </td>
								</tr>
							</thead>
							<tbody>
							<%	
								try{
									ResultSet rs = stmtQry.executeQuery("SELECT * FROM dbworkcode ORDER BY work_code ASC");
									while(rs.next()){								
										String work_code = rs.getString("work_code");
							%>
								<tr>
							<%	if(checkPermission(ses_per, "01")){	%>			
									<td align="center"> 
										<a href="edit_workcode.jsp?action=edit&work_code=<%= work_code %>"> <img src="images/edit.png" width="18" height="18" border="0" align="absmiddle" data-toggle="tooltip" data-placement="right" title="<%= lb_editdata %>"> </a>
									</td>
									<td align="center"> 
										<img src="images/delete.png" width="20" height="20" border="0" align="absmiddle" onClick="ModalConfirm('<%= work_code %>', '<%= msg_confirmdel %>', '<%= rs.getString("th_desc") %>', '<%= rs.getString("en_desc") %>');" data-toggle="tooltip" data-placement="right" title="<%= lb_deletedata %>">
									</td>
							<%	}	%>
									<td align="center"> <b> <a href="#" onClick="show_detail('<%= work_code %>');" data-toggle="tooltip" data-placement="right" data-html="true" title="<%= lb_viewdata %>"> <%= work_code %> </a> </b> </td>
									<td class="pad-left-10"> <%= rs.getString("th_desc") %> </td>
									<td class="pad-left-10"> <%= rs.getString("en_desc") %> </td>
									<td class="pad-left-10"> <%= rs.getString("map_output") %> </td>
								</tr>
							<%	
									}
									rs.close();
					...
					<div class="col-xs-12" align="left" style="margin-left: -15px; min-width: 500px;">
						<div class="btn-group">
							...
							<%	if(checkPermission(ses_per, "01")){	%>	
							<button type="button" class="btn btn-primary btn-sm" onClick="Add_Data('edit_workcode.jsp?action=add');"> 
							...
							<div class="col-xs-12 col-md-12" style="margin-bottom: 15px;">
								<strong>
									<div class="alert alert-warning" role="alert" style="min-width: 50px;">
										<div class="row">
											<div class="col-xs-5 col-md-5" align="right"> <%= lb_workcode %> :  </div>
		...
		<script>
		function show_detail(work_code){
			view_detail_140px.location = 'view_workcode.jsp?action=view&work_code='+work_code;
