...
<%	
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			sql = sql + "door.door_id, emp.sec_code "
			...
			try{
				...
				while(rs.next()){
					...
					if(!(tmp_id.equals(idcard))){
						count_emp++;
						sqlTmp = getSQLInsertTmp102(TempName,rec,idcard,emp_name,emp_date,emp_day,
									emp_time,emp_duty,emp_status,emp_door,emp_event);
					}else{
						if(!(tmp_date.equals(emp_date))){
							sqlTmp = getSQLInsertTmp102(TempName,rec,"","",emp_date,emp_day,
										emp_time,emp_duty,emp_status,emp_door,emp_event);
						}else{
							sqlTmp = getSQLInsertTmp102(TempName,rec,"","","","",
									emp_time,emp_duty,emp_status,emp_door,emp_event);
						}
					}
			...
			String fileName = report_id+"_All_Transaction.xls";
			int num = 0;
			int num_id = 0;		
			try{	
				...
				row_head.createCell((short)7).setCellValue(lb_doors); 
				...
				try{
					...
					while(rs.next()){		
						...
						row_query_db.createCell((short)7).setCellValue(emp_door);
