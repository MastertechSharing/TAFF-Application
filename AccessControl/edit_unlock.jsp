...
<% 
	...
	session.setAttribute("action", "edit_unlock.jsp?"+"&action="+request.getParameter("action")+"&day_type="+request.getParameter("day_type")+"&");        
%>
...
<html lang="en">
	<head>
		...
		<script language="javascript">
			...
			function SelectGroup(txt, lang){
				//	var chkg = document.form1.group1.value;
				if(document.form1.group1.checked == true){
					if(document.form1.group2.checked == true){
						ModalWarning_TextName(txt, "");
						document.form1.group1.checked = false;
						return false;
					}               
				}
			}
			
			function SelectGroup2(txt, lang){
				//	var chkg = document.form1.group1.value; 
				if(document.form1.group2.checked == true){
					if(document.form1.group1.checked == true){
						ModalWarning_TextName(txt, "");
						document.form1.group2.checked = false;
						return false;
					}               
				}
			}
			
			function ConfirmEditTimeUnlock(act, url, sText, msg_time1, msg_chk_mistaketime1, msg_time2, msg_chk_mistaketime2, msg_time3, msg_chk_mistaketime3, msg_time4, msg_chk_mistaketime4, msg_time5, msg_chk_mistaketime5, txttime){
				//	time1
				...
				if(time1 != ''){
					if(chkEmptyTime(hh1, mm1, hh2, mm2)){
						ModalWarning_TextName(msg_time1, "");
						return false;
					}else{                  
						if(comparetime(hh1, mm1, hh2, mm2)){
							ModalWarning_TextName(msg_chk_mistaketime1, "");
							return false;
						}
					}
				}
				...
				if(time2 != ''){
					if(chkEmptyTime(hh3, mm3, hh4, mm4)){
						ModalWarning_TextName(msg_time2, "");
						return false;
					}else{          
						if(comparetime(hh3, mm3, hh4, mm4)){
							ModalWarning_TextName(msg_chk_mistaketime2, "");
							return false;
						}
					}
				}
				...
				if(time3 != ''){
					if(chkEmptyTime(hh5, mm5, hh6, mm6)){
						ModalWarning_TextName(msg_time3, "");
						return false;
					}else{
						if(comparetime(hh5, mm5, hh6, mm6)){
							ModalWarning_TextName(msg_chk_mistaketime3, "");
							return false;
						}
					}
				}
				...
				if(time4 != ''){
					if(chkEmptyTime(hh7, mm7, hh8, mm8)){
						ModalWarning_TextName(msg_time4, "");
						return false;
					}else{
						if(comparetime(hh7, mm7, hh8, mm8)){
							ModalWarning_TextName(msg_chk_mistaketime4, "");
							return false;
						}
					}
				}
				...
				if(time5 != ''){
					if(chkEmptyTime(hh9, mm9, hh10, mm10)){
						ModalWarning_TextName(msg_time5, "");
						return false;
					}else{
						if(comparetime(hh9, mm9, hh10, mm10)){
							ModalWarning_TextName(msg_chk_mistaketime5, "");
							return false;
						}
					}
				}
						...
						if(hh5 == ""&&mm5 == ""&&hh6 == ""&&mm6 == ""){
							if(hh7 == ""&&mm7 == ""&&hh8 == ""&&mm8 == ""){
								if(hh9 == ""&&mm9 == ""&&hh10 == ""&&mm10 == ""){
									if(document.form1.group1.checked == true || document.form1.group2.checked == true){
...
<% 
		...
		if(request.getParameter("action") != null){
			...
			response.sendRedirect("data_unlock.jsp");
		}
...
<%  	if(action.equals("edit")){                      
			...
			try{
				ResultSet rs = stmtQry.executeQuery("SELECT * FROM dbunlock WHERE (day_type = '"+day_type+"') ");
				while(rs.next()){                                       
				...
				<%	if(typeday != 8){	%>
						<div class="row form-group">
							...
							<div class="col-md-10" style="margin-top: 6px;"> 
								<input type="checkbox" name="group1" value="<%= group1 %>" <% if(group1.equals("1")){ out.println("checked"); } %> onClick="SelectGroup('<%= lb_mgroup %>', '<%= lang %>');"/> &nbsp; <%=lb_mgroup1%>
								<input type="hidden" name="hidgroup1" id="hidgroup1" value="<%= group1 %>">
							</div>
						</div>  
						<div class="row form-group">
							<div class="col-md-2"> </div>
							<div class="col-md-10" style="margin-top: 6px;">
								<input type="checkbox" name="group2" value="<%= group2 %>" <% if(group2.equals("1")){ out.println("checked"); } %> onClick="SelectGroup2('<%= lb_mgroup %>', '<%= lang %>');"/> &nbsp; <%=lb_mgroup2%>
								<input type="hidden" name="hidgroup2" id="hidgroup2" value="<%= group2 %>">
							</div>
						</div>  
				<%	}else{	%>
						<input type="hidden" name="group1" id="group1">
						<input type="hidden" name="hidgroup1" id="hidgroup1" value="0">
						<input type="hidden" name="group2" id="group2">
						<input type="hidden" name="hidgroup2" id="hidgroup2" value="0">
				<%	}	%>
						<div class="row form-group" style="margin-bottom: 0px;">
							<div class="col-md-12" align="center"> 
								<input type="button" class="btn btn-primary btn-sm button-shadow1 button-shadow2" id="btnok" value=" &nbsp; &nbsp; &nbsp; <%= btn_ok %> &nbsp; &nbsp; &nbsp; " onClick="ConfirmEditTimeUnlock('edit','module/act_unlock.jsp?action=edit','<%=msg_confirmedit%>','<%=msg_input_time1%>','<%=msg_chk_mistaketime1%>','<%=msg_input_time2%>','<%=msg_chk_mistaketime2%>','<%=msg_input_time3%>','<%=msg_chk_mistaketime3%>','<%=msg_input_time4%>','<%=msg_chk_mistaketime4%>','<%=msg_input_time5%>','<%=msg_chk_mistaketime5%>','<%=lb_msg_time%>');"> &nbsp; 
								<input type="button" class="btn btn-default btn-sm button-shadow1 button-shadow2" value=" &nbsp; &nbsp; &nbsp; <%= btn_cancel %> &nbsp; &nbsp; &nbsp; " onClick="location.href='data_unlock.jsp'">
							</div>
						</div>  
					...
					<div class="modal-body" align="center">
						<div class="table-responsive" style="border: 0px !important;" border="0"> 
							...
							<div class="col-xs-12 col-md-12" style="margin-bottom: 15px;"> 
								...
								<div class="col-xs-5 col-md-5">
									<button type="button" class="btn btn-default btn-sm button-shadow1 button-shadow2" onClick="javascript: location.href = 'data_unlock.jsp';" style="width: 100%;"> <%= btn_cancel %> </button>
								</div>
								<div class="col-xs-1 col-md-1"> 
