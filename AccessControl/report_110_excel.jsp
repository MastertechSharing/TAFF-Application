...
<% 
	...
	session.setAttribute("action", "report_110_excel.jsp?");
%>
...
<%	
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			String sql = "SELECT trs.*, door.door_id, door.group_door, loc.locate_code, ";
			if(mode == 0){
				sql = sql + "date_format(trs.date_event,'%d/%m/%Y') AS date_work, ";
			}else{
				sql = sql + "convert(varchar(10),trs.date_event,103) AS date_work, ";
			}
			if(lang.equals("th")){
			...
			sql = sql + "emp.sec_code "
					...
					+ "LEFT OUTER JOIN dblocation loc ON (door.locate_code = loc.locate_code) "
			...
			try{
				...
				while(rs.next()){
					...
					emp_locate = rs.getString("locate_code");
					...
					switch(count){
						case 0:
							sqlTmp = getSQLInsertTmp110(TempName,rec,idcard,emp_name,emp_date,
										emp_time,emp_door,emp_duty,emp_locate); 
							break;
						case 1:
							sqlTmp = getSQLInsertTmp110(TempName,rec,"","",emp_date,
										emp_time,emp_door,emp_duty,emp_locate); 
							break;
						case 2:
							sqlTmp = getSQLUpdateTmp110(TempName,rec,idcard,emp_name,emp_date,
										emp_time,emp_door,emp_duty,emp_locate);
							break;
					}
			...
			try{
				...
				row_head.createCell((short)4).setCellValue(lb_date);
				row_head.createCell((short)5).setCellValue(lb_time_trans);  
				row_head.createCell((short)6).setCellValue(lb_readerno); 
				row_head.createCell((short)7).setCellValue(lb_reportstatus); 
				row_head.createCell((short)8).setCellValue(lb_locatecode); 
				row_head.createCell((short)9).setCellValue(lb_comment); 
				
				try{
					String sql_where = "";
					if(!duty.equals("")){
						sql_where = " WHERE last_status = '"+duty+"' ";
					}
					sql = "SELECT tmp.*, emp.sec_code, sec.th_desc AS sec_th_desc, sec.en_desc AS sec_en_desc, "
					...
					while (rs.next()) {
						...
						String sec_desc = "";				
						if(lang.equals("th")){
							sec_desc = rs.getString("sec_th_desc");								
						}else{
							sec_desc = rs.getString("sec_en_desc");				
						}	
						String loc_desc = "";				
						if(lang.equals("th")){
							loc_desc = rs.getString("loc_th_desc");								
						}else{
							loc_desc = rs.getString("loc_en_desc");				
						}					
						num++;					
						...
						row_query_db.createCell((short)4).setCellValue(emp_date);
						row_query_db.createCell((short)5).setCellValue(rs.getString("last_time"));
						row_query_db.createCell((short)6).setCellValue(rs.getString("reader_no"));
						row_query_db.createCell((short)7).setCellValue(rs.getString("last_status"));
						if(rs.getString("locate_code") != null){
							row_query_db.createCell((short)8).setCellValue(rs.getString("locate_code")+" : "+loc_desc);
						}else{
							row_query_db.createCell((short)8).setCellValue("");
						}
