...
<% 
	...
	session.setAttribute("subtitle", "report_105");
	session.setAttribute("action", "report_105.jsp?");
		...
		<script>
			...
			$(document).ready(function() {
				...
				$('#btn_action button').on('click', function(){
					...
					fncSubmit('<%= lang %>', '<%= lb_no_datareader %>', '<%= msg_ps_selectreader %>', '<%= lb_no_datasection %>', '<%= msg_ps_selectsection %>', cRow_door, cRow_section, '');
					return false;
				});
			...
			function can_not_idcard(cRow_section){
				if(chk_cRow_section != cRow_section){
					if($('#emp_id').val() != ''){
						ModalWarning_TextName('<%= lb_selsec_not_selemp %>', '');
						$('#emp_id').val('');
						$('#emp_name').val('');
					}
				}
							...
							<div class="row" style="margin-left: 0px; margin-right: 0px;">
								<div class="modal-title col-xs-12 col-md-12">
									...
									<div class="bs-callout bs-callout-info"> 
										...
										<p>
										<div class="row">
											<h5 class="modal-title col-xs-2 col-md-2" style="margin-top: 4px;"> <div align="right"> <b> <%= lb_mm_yy %> : </b> </div> </h5>
											<div class="modal-title col-xs-2 col-md-2"> 
										<%	
											String d1 = getCurrentDate();
											int iDay = valueToInt(request.getParameter("today"));  
											int iMonth = valueToInt(request.getParameter("month"));
											int iYear = valueToInt(request.getParameter("year"));
										...
										%> 
												<select name="month" id="month" class="form-control" style="max-height: 28px !important; font-size: 12px !important;">
											<%	for(int i_mm = 1; i_mm <= 12; i_mm++){		
													if(i_mm == iMonth){											
											%>		<option value="<%= i_mm %>" selected="selected"> <%= getLongMonth(i_mm, lang) %> </option>
											<%		}else{	%>
													<option value="<%= i_mm %>"><%= getLongMonth(i_mm, lang) %> </option>
											<%		}
												}
											%>
												</select>
											...
											<div class="modal-title col-xs-1 col-md-1">
												<label style="font-size: 20px; margin-left: -18px; margin-top: 0px;"> / </label>
												<select name="year" id="year" class="form-control" style="max-height: 28px !important; font-size: 12px !important; min-width: 80px; margin-top: -33px;">
											<%	//	start year and end year in combo box to change year in calendar
												for(int iy = iTYear -10; iy <= iTYear +10; iy++){
													if(iy == iYear){
											...
											%>
												...
												<input type="hidden" name="today" value="<%= d1.substring(0, 2) %>">
											</div>
											<div class="modal-title col-xs-6 col-md-6" align="right">
												<div class="btn-group" id="btn_action">
													<div class="btn-group dropup">
														<button class="btn btn-primary btn-sm max-height-26 button-shadow1 button-shadow2" name="btn_pdf" id="btn_pdf" onClick="javascript: $('#pagefile').val('pdf');" onMouseOver="this.style.cursor='hand';"> &nbsp; &nbsp; &nbsp; &nbsp; <strong> <%= lb_viewdata %> </strong> &nbsp; &nbsp; &nbsp; </button>
													</div>
											...
											<%	String rdfc = "";							
												ResultSet rsR = stmtQry.executeQuery("SELECT readerf FROM dbcompany");
												while(rsR.next()){
													rdfc = Integer.toString(rsR.getInt("readerf"));
												}	rsR.close();
											%>	<input type="hidden" name="rdfunc" id="rdfunc" value="<%= rdfc %>">	
												
											<%	if(checkPermission(ses_per, "9")){	%>
												<input type="hidden" name="hiddenReader" id="hiddenReader" value="all">		  
											<%	}else{	%>
												<input type="hidden" name="hiddenReader" id="hiddenReader" value="">
											<%	}	%>	
											</div> 
											<div class="modal-title col-xs-1 col-md-1"> </div> 
										</div>
								...
								<div class="modal-title col-xs-12 col-md-12">
									...
									<div class="bs-callout bs-callout-info"> 
										...
										<div class="row" style="margin-top: -25px;">
											...
											<table class="table table-bordered table-hover" id="data_reader" align="center" border="0" style="max-width: 98% !important; margin-bottom: -5px;">
												<thead>
													<tr>
														<th width="10%" style="text-align: center;"> </th>
														<th width="90%" style="text-align: center;"> <% if(lang.equals("th")){ out.println(lb_thdesc); }else{ out.println(lb_endesc); } %> </th>
													</tr>
												</thead>
												<tbody>
											<%	try{
											...
											%>		
													<tr>
														...
														<td class="pad-left-10"> 
															<div class="ellipsis_string" style="max-width: 900px;">
		...
		<script>
			...
			function fncSubmit(lang, msg, msg2, msg3, msg4, cRow_door, cRow_section, supervisor){
				...
				if(document.form1.emp_id.value == ""){
					if(lang == "th"){				
						ModalWarning_TextName('กรุณาเลือกพนักงาน', '');
					}else{
						ModalWarning_TextName('Please Select Employee', '');
					}
					return false;
				}
				...
				if(document.form1.pagefile.value == "pdf"){		
					document.form1.action = "report_105_pdf.jsp";
