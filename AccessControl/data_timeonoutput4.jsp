...
<% 
	...
	session.setAttribute("action", "data_timeonoutput4.jsp?action="+getaction+"&");
...
<html lang="en">
	<head>
		...
		<script type="text/javascript">
			...
			function Confirm_Button(){
				location.href = 'module/act_timeonoutput4.jsp?action=clear&day_type='+document.getElementById('sCode').value;
			}
		</script>
	</head>
	<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onLoad="Clear_Session_Table('<%= clear_session %>', 'DataTables_data_table_<%= request.getRequestURI() %>'); Load_DataTable();">
					...
					<div style="min-width: 1080px;" class="table" border="0">
						...
						<table style="min-width: 1080px;" align="center" id="data_table" class="table table-striped table-bordered" cellspacing="0" >
							...
							<%	if(checkPermission(ses_per, "01")){	%>	
									...
									<td width="3%" align="center"> </td>
							<%	}	%>
									<td width="24%" align="center"> <%= lb_day %> </td>
							...
							<%	
								try{
									ResultSet rs = stmtQry.executeQuery("SELECT * FROM dbtimeon_out4 ORDER BY day_type ASC");
									while(rs.next()){
							...
							<%	if(checkPermission(ses_per, "01")){	%>			
									<td align="center"> 
										<a href="edit_timeonoutput4.jsp?action=edit&day_type=<%= day_type %>"> <img src="images/edit.png" width="18" height="18" border="0" align="absmiddle" data-toggle="tooltip" data-placement="right" title="<%= lb_editdata %>"> </a>
									</td>
									<td align="center"> 
							...
							<%	}	%>
									...
									<td align="center"> <%= changeTimeSec(time2) %> </td>
									<td align="center"> <%= changeTimeSec(time3) %> </td>
									<td align="center"> <%= changeTimeSec(time4) %> </td>
									<td align="center"> <%= changeTimeSec(time5) %> </td>
								</tr>
							<%	
									}
									rs.close();
		...
		function show_detail(day_type){
			view_detail.location = 'view_timeonoutput4.jsp?action=view&day_type='+day_type;
