...
<%    
	...
	session.setAttribute("subtitle", "getholiday");
	String ip = "", door_id = "";
	...
	session.setAttribute("action", "cmd_get_holiday_data.jsp?ip="+ip+"&door_id="+door_id+"&");	
%>
<span/>
<html lang="en">
	<head>
...
<%  
	...
	try{
		...
		String pathfile = "";
		if(!ip.equals("")){
			pathfile = path_data+ip+"\\HOLIDAY.CFG";
		}else{
			pathfile = path_data+door_id+"\\HOLIDAY.CFG";
		}
		File f = new File(pathfile);
		if(!f.exists()){
			out.println("<div class='alert alert-danger' role='alert'>");
			out.println("	<span class='alert-link'> "+msg_not_file_upload+" "+lb_getholiday+" "+lb_doorcode+" : "+door_id+" - "+desc+" </span>");
			out.println("</div>");
		}else{
%>
				<div class="alert alert-info" role="alert">
					<span class="alert-link"> <%= lb_getholiday %> <%= lb_doorcode %> : <%= door_id %> - <%= desc %> </span>
				</div>
				...
				<table style="min-width: 800px;" class="table table-hover" align="center" id="table1" border="0">
					<thead>
						<tr>
							...
							<td width="20%" align="center"> <label class="control-label"> <%= lb_date %> </div> </td>
							<td width="35%" align="center"> <label class="control-label"> <%= lb_thdesc %> </div> </td>
							<td width="35%" align="center"> <label class="control-label"> <%= lb_endesc %> </div> </td>
						</tr>
					</thead>
					</tbody>					
<%		
			...
			String textStr = "";
			String yyyy = "", mm = "", dd = "", holi_date = "", th_desc = "", en_desc = "";
			int i = 1 ;
			while((textStr = in.readLine()) != null){
				if((textStr.length() == 8) && (!textStr.equals("********"))){
					yyyy = textStr.substring(0, 4);
					mm = textStr.substring(4, 6);
					dd = textStr.substring(6, 8);
					holi_date = (yyyy+"-"+mm+"-"+dd);
					
					rs = stmtQry.executeQuery("SELECT * FROM dbholiday WHERE (holi_date='"+holi_date+"')");
					while(rs.next()){
