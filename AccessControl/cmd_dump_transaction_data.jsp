...
<% 
	...
	session.setAttribute("subtitle", "dumptransaction");
	String ip = "", door_id = "";
	...
	session.setAttribute("action", "cmd_dump_transaction_data.jsp?ip="+ip+"&door_id="+door_id+"&row_data="+row_data+"&");
	...
	String pathfile = "";
	if(!ip.equals("")){
		pathfile = path_data + ip + "\\DUM\\DUM.REC";
	}else{
		pathfile = path_data + door_id + "\\DUM\\DUM.REC";
	}
...
<%	
	try{
		...
		if(!f.exists()){
			...
			out.println("	<span class='alert-link'> "+msg_not_file_upload+" "+lb_cmd57+" "+lb_doorcode+" : "+door_id+" - "+desc+" </span>");
			out.println("</div>");
		}else{
...
%>
				<div class="alert alert-info" role="alert" style="min-height: 50px;">
					<span class="modal-title col-xs-5 col-md-5" style="margin-top: -10px;">
						<span class="alert-link"> <%= lb_cmd57 %> <br/> <%= lb_doorcode %> : <%= door_id %> - <%= desc %> </span>
					</span>
...
%>  
		...
		<script>
			...
			function saveFile(ip, door_id){
				if(ip != ''){
					download_file.location.href = 'cmd_dump_transaction_data.jsp?ip='+ip+'&download=1&';
				}else{
					download_file.location.href = 'cmd_dump_transaction_data.jsp?door_id='+door_id+'&download=1&';
				}
			...
			//	document.getElementById('space').style.display = 'none';
				process_result.location.href = 'module/transaction_to_database.jsp?ip='+ip+'&door_id='+door_id+'&subfolder=DUM&filename=DUM.REC&pages=process';
			...
			function ProcessToFormat(ip, door_id, sText){
				...
				process_result.location.href = 'module/transaction_to_dataformat.jsp?ip='+ip+'&door_id='+door_id+'&subfolder=DUM&filename=DUM.REC&pages=process';
				document.getElementById('processing').innerHTML = sText;
				$('#myModalWait').modal('show');
			}
			...
			function ProcessToFileTAF(ip, door_id, sText){
				...
				process_result.location.href = 'module/transaction_to_filetaf.jsp?ip='+ip+'&door_id='+door_id+'&subfolder=DUM&filename=DUM.REC&pages=process';
				document.getElementById('processing').innerHTML = sText;
				$('#myModalWait').modal('show');
			}
			
			function reload(ip, door_id){
				location.href = "cmd_dump_transaction_data.jsp?ip="+ip+"&door_id="+door_id+"&row_data="+document.getElementById("row_data").value+"&";
			}
