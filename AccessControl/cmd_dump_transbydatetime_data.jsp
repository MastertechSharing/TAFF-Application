...
<% 
	...
	session.setAttribute("action", "cmd_dump_transbydatetime_data.jsp?ip="+ip+"&door_id="+door_id+"&row_data="+row_data+"&");
	...
	String pathfile = "";
	if(!ip.equals("")){
		pathfile = path_data + ip + "\\DUM\\";
	}else{
		pathfile = path_data + door_id + "\\DUM\\";
	}
	...
	if(aDirectory.exists()){
		for(int i = 0; i < filesInDir.length; i++){
			if(filesInDir[i].substring((filesInDir[i].length() -3), filesInDir[i].length()).equals("REC")){
	...
	if(request.getParameter("download") != null){
		...
		try{
			...
			myfile = new File(pathfile + filename);
			if (myfile.exists()){
				// set response headers
				response.setContentType("text/plain");
				response.addHeader("Content-Disposition", "attachment; filename="+door_id+filename.substring(3, (filename.length() -4))+".REC");
...
<%	
	try{
		...
		if(!f.exists()){
			...
			out.println("	<span class='alert-link'> "+msg_not_file_upload+" "+lb_cmd69+" "+lb_doorcode+" : "+door_id+" - "+desc+" </span>");
			out.println("</div>");
		}else{
			
			BufferedReader in = new BufferedReader(new FileReader(pathfile + filename));
			String textStr = "";
			int row_trans = 0;
...
%>
				<div class="alert alert-info" role="alert" style="min-height: 50px;">
					<span class="modal-title col-xs-5 col-md-5" style="margin-top: -10px;">
						<span class="alert-link"> <%= lb_cmd69 %> <br/> <%= lb_doorcode %> : <%= door_id %> - <%= desc %> </span>
					</span>
					<span class="modal-title col-xs-7 col-md-7" style="text-align: right;">
						<div class="btn-group">
							...
							<button class="btn btn-default btn-xs button-shadow1 button-shadow2" id="btn_process" onClick="ProcessToBase('<%= ip %>', '<%= door_id %>', '<%= filename %>', '<%= lb_processing_to_base %>');" onMouseOver="this.style.cursor='hand';"> 
							...
							<button class="btn btn-default btn-xs button-shadow1 button-shadow2" id="btn_format" onClick="ProcessToFormat('<%= ip %>', '<%= door_id %>', '<%= filename %>', '<%= lb_processing_to_format %>');" onMouseOver="this.style.cursor='hand';"> 
							...
							<button class="btn btn-default btn-xs button-shadow1 button-shadow2" id="btn_taf" onClick="ProcessToFileTAF('<%= ip %>', '<%= door_id %>', '<%= filename %>', '<%= lb_processing_to_taf %>');" onMouseOver="this.style.cursor='hand';"> 
...
%>  
		...
		<script>
			...
			function ProcessToBase(ip, door_id, filename, sText){
				document.getElementById("btn_process").disabled = true;
			//	document.getElementById('img_load').style.display = '';
			//	document.getElementById('space').style.display = 'none';
				process_result.location.href = 'module/transaction_to_database.jsp?ip='+ip+'&door_id='+door_id+'&subfolder=DUM&filename='+filename+'&pages=process';
			...
			function ProcessToFormat(ip, door_id, filename, sText){
				document.getElementById("btn_format").disabled = true;
				process_result.location.href = 'module/transaction_to_dataformat.jsp?ip='+ip+'&door_id='+door_id+'&subfolder=DUM&filename='+filename+'&pages=process';
				document.getElementById('processing').innerHTML = sText;
				$('#myModalWait').modal('show');
			}
		
			function ProcessToFileTAF(ip, door_id, filename, sText){
				document.getElementById("btn_taf").disabled = true;
				process_result.location.href = 'module/transaction_to_filetaf.jsp?ip='+ip+'&door_id='+door_id+'&subfolder=DUM&filename='+filename+'&pages=process';
				document.getElementById('processing').innerHTML = sText;
				$('#myModalWait').modal('show');
			}
			
			function reload(ip, door_id){
				location.href = "cmd_dump_transbydatetime_data.jsp?ip="+ip+"&door_id="+door_id+"&row_data="+document.getElementById("row_data").value+"&";
			}
