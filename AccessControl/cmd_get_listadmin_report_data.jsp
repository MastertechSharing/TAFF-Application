...
<%	
	...
	session.setAttribute("subtitle", "getlistadmin");
	session.setAttribute("action", "cmd_get_listadmin.jsp?");
	String ip = "", door_id = "";
...
<%	
	...
	try{
		...
		String pathfile = "";
		if(!ip.equals("")){
			pathfile = path_data+ip+"\\ListAdmin.txt";
		}else{
			pathfile = path_data+door_id+"\\ListAdmin.txt";
		}
		File f = new File(pathfile);
		if(!f.exists()){
			out.println("<div class='alert alert-danger' role='alert' style='max-height: 32px;'>");
			out.println("	<div class='alert-link' style='margin-top: -10px;'> "+msg_not_file_upload+" "+lb_cmd88+" "+lb_doorcode+" : "+door_id+" - "+desc+" </div>");
			out.println("</div>");
		}else{
%>
				<div class="alert alert-info" role="alert" style="max-height: 32px;">
					<div class="alert-link" style="margin-top: -10px;"> <%= lb_cmd88 %> <%= lb_doorcode %> : <%= door_id %> - <%= desc %> </div>
				</div>
				...
				<table style="min-width: 500px; margin-top: -15px; margin-bottom: -15px;" class="table table-hover" align="center" id="table1" border="0">
					<thead>
						<tr>
							...
							<td width="35%" align="center"> <label class="control-label"> <%= lb_empcode %> </label> </td>
							<td width="55%" align="center"> <label class="control-label"> <%= lb_names %> </label> </td>
						</tr>
					</thead>
					<tbody>
<%					
			...
			while((textStr = in.readLine()) != null){
				if(textStr.length() >= 1){
					...
					String fullname = "";
					String sql_name = " SELECT "+lang+"_fname AS fname, "+lang+"_sname AS sname FROM dbemployee WHERE idcard = '"+textStr.trim()+"' ";
					ResultSet rs_name = stmtTmp.executeQuery(sql_name);
					if(rs_name.next()){
						fullname = rs_name.getString("fname")+" "+rs_name.getString("sname");
					}	rs_name.close();
					
%>
          <tr>
			<td align="center"> <%= sum %> </td>
			<td align="center"> <%= textStr %> </td>
            <td class="pad-left-10"> <%= fullname %> </td>
          </tr>
<%
				}
			}
