...
<% 
	...
	session.setAttribute("action", "report_101_pdf.jsp?");
...
<html lang="en">
	<head>
		...
		<script>
			function orderBy(){
				document.form1.action = "report_101_pdf.jsp";
				document.getElementById("form1").submit();
			}
			function changePage(screen){
				document.form1.action = "report_101_pdf.jsp?screen="+screen+"&";
				document.getElementById("form1").submit();
			}
		</script>
...
<%	
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			if(db_type == 0){
				sql = sql + "date_format(trs.datetime_update,'%d/%m/%Y %T') AS dt_update, ";
			}else{
				sql = sql + "(convert(varchar(10),trs.datetime_update,103)+' '+convert(varchar(8),trs.datetime_update,108)) AS dt_update, ";	
			}
			if(lang.equals("th")){
								...
								<div class="modal-title col-xs-12 col-md-12">
									...
									<div class="bs-callout bs-callout-info"> 
										<div class="row alert-message-info" style="height: 32px; margin-left: -10px; margin-top: -10px; margin-right: -10px;">
											<label class="col-xs-6 col-md-6 control-label" style="margin-top: 5px;"> <i class="glyphicon glyphicon-list-alt"> </i> &nbsp; <a href="report_101.jsp"> <%= lb_report_101 %> </a> </label>
							...
							<div class="modal-title col-xs-12 col-md-12">
								...
								<div class="bs-callout bs-callout-info"> 
									<div class="row" align="center">
										<iframe name="show_body" width="97%" height="700" src="ReportTimeStamp.do?st_date=<%= date1 %>&ed_date=<%= date2 %>&time1=<%= time1 %>&time2=<%= time2 %>&emp_id=<%= emp_id %>&select_section=<%= section %>&start=<%= startRow %>&rep_id=<%= report_id %>&lang=<%= lang %>" frameborder="0" scrolling="yes"></iframe>
									</div>
								</div>
