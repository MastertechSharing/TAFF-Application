...
<% 
	...
	session.setAttribute("subtitle", "position"); 
	session.setAttribute("action", "edit_position.jsp?"+"&action="+request.getParameter("action")+"&pos_code="+request.getParameter("pos_code")+"&");	
%>
	...
	<head>
		...
		<script language="javascript">
			...
			function change_class(field_name){
				if(field_name == 'pos_code'){
					if(document.form1.pos_code.value != ""){
						document.getElementById("textbox_pos_code").className = "form-group has-success has-feedback";
						document.getElementById("icon_pos_code").className = "glyphicon glyphicon-ok form-control-feedback";
					}else{
						document.getElementById("textbox_pos_code").className = "form-group has-error has-feedback";
						document.getElementById("icon_pos_code").className = "glyphicon glyphicon glyphicon-star form-control-feedback";
					}
				}
			}
		</script>
...
<% 
		...
		if(request.getParameter("action") != null){
			...
			response.sendRedirect("data_position.jsp");
		}
...
<% 		if(action.equals("add")){	%>
						...
						<div class="row form-group">
							<label class="control-label label-text-1 col-md-2"> <%= lb_poscode %> : </label>
							<div class="col-md-4">
								<div class="form-group has-error has-feedback" id="textbox_pos_code" style="margin-bottom: 0px">
									<input type="text" class="form-control" id="pos_code" name="pos_code" maxlength="6" placeholder="<%= lb_poscode %>" onKeyPress="IsValidCharacter()" onKeyUp="change_class('pos_code');" onBlur="change_class('pos_code');">
						...
						<div class="row form-group" style="margin-bottom: 0px;">
							<div class="col-md-12" align="center"> 
								<input type="button" class="btn btn-primary btn-sm button-shadow1 button-shadow2" id="btnok" value=" &nbsp; &nbsp; &nbsp; <%= btn_ok %> &nbsp; &nbsp; &nbsp; " onClick="return Confirm_add_edit(document.form1.pos_code, '<%= msg_confirmedit %>', '<%= msg_notinput_pos %>', 'module/act_position.jsp?action=add', 'add');"> &nbsp; 
								<input type="button" class="btn btn-default btn-sm button-shadow1 button-shadow2" value=" &nbsp; &nbsp; &nbsp; <%= btn_cancel %> &nbsp; &nbsp; &nbsp; " onClick="location.href='data_position.jsp'">
							</div>
						</div> 
...
<% 		}else if(action.equals("edit")){			
			...
			try{
				ResultSet rs = stmtQry.executeQuery("SELECT * FROM dbposition WHERE (pos_code = '"+pos_code+"') ");
				while(rs.next()){				
%>
				...
				<div class="row form-group">
					<label class="control-label label-text-1 col-md-2"> <%= lb_poscode %> : </label>
					<div class="col-md-4">
						<div class="form-group has-success has-feedback" id="textbox_pos_code" style="margin-bottom: 0px">
							<input type="text" class="form-control" id="pos_code" name="pos_code" value="<%= pos_code %>" maxlength="6" placeholder="<%= pos_code %>" onKeyPress="IsValidCharacter()" onKeyUp="change_class('pos_code');" onBlur="change_class('pos_code');">
							<span class="glyphicon glyphicon-ok form-control-feedback" id="icon_pos_code" aria-hidden="true"> </span>
							<input type="hidden" name="pos_code2" id="pos_code2" value="<%= pos_code %>">
						</div>
					</div>
					<div class="col-md-6" style="margin-top: 6px;"> </div>
				</div>
				...
				<div class="row form-group" style="margin-bottom: 0px;">
					<div class="col-md-12" align="center"> 
						<input type="button" class="btn btn-primary btn-sm button-shadow1 button-shadow2" id="btnok" value=" &nbsp; &nbsp; &nbsp; <%= btn_ok %> &nbsp; &nbsp; &nbsp; " onClick="return Confirm_add_edit(document.form1.pos_code, '<%= msg_confirmedit %>', '<%= msg_notinput_pos %>', 'module/act_position.jsp?action=edit', 'edit');"> &nbsp; 
						<input type="button" class="btn btn-default btn-sm button-shadow1 button-shadow2" value=" &nbsp; &nbsp; &nbsp; <%= btn_cancel %> &nbsp; &nbsp; &nbsp; " onClick="location.href='data_position.jsp'">
					</div>
				</div> 						
<%			}	
