...
<% 
	...
	session.setAttribute("subtitle", "report_402");
	session.setAttribute("action", "report_402_pdf.jsp?file="+request.getParameter("file")+"&");
...
<html lang="en">
		...
		if (!(file == null || file.equals(""))) {
			try{
				...
				if(checkPermission(ses_per, "12")){
					sql += "INNER JOIN dbsection s ON (s.sec_code = emp.sec_code) ";
					sql += "INNER JOIN dbusers u ON (u.dep_code = s.dep_code) ";
				}else if(checkPermission(ses_per, "56")){
					sql += "INNER JOIN dbsection s ON (s.sec_code = emp.sec_code) ";
					sql += "INNER JOIN dbusers u ON (u.sec_code = s.sec_code) ";
				}
								...
								<div class="modal-title col-xs-12 col-md-12">
									<div class="bs-callout bs-callout-info">
										<div class="row alert-message-info" style="height: 32px; margin-left: -10px; margin-top: -10px; margin-right: -10px; margin-bottom: 10px;">
											<label class="col-xs-6 col-md-6 control-label" style="margin-top: 5px; text-align: left;"> <i class="glyphicon glyphicon-list-alt"> </i> &nbsp; <%= lb_righofdoor_q %> </label>
										...
										<div class="row">
											...
											<div class="col-xs-4 col-md-4"> 
												<select class="form-control <%= selectpicker %>" data-live-search="true" data-width="100%" data-size="12" data-container="body" name="select_door" onChange="selectBy();">		
										<%										
											try{
										...
										%>		</select>
											...
											<div class="col-xs-4 col-md-4"> 
												<select class="form-control <%= selectpicker %>" data-live-search="true" data-width="100%" data-size="12" data-container="body" name="select_section" id="select_section" onChange="selectBy();">
													<option value="0"> <%=lb_sectioall%> </option>
										<%	
											...
											try{
												...
												while(rs1.next()){
													if(select_sect.equals("")){
														select_sect = "0";//rs1.getString("sec_code");
													}
										%>	
										...
										%>		</select>
												<input type="hidden" id="hiddoor_id" name="hiddoor_id" value="InOutByDoorQuick">
											</div>
							...
							<div class="modal-title col-xs-12 col-md-12">
								...
								<div class="bs-callout bs-callout-info"> 
									<div class="row" align="center">
										<iframe name="show_body" width="97%" height="700" src="ReportRightInOut.do?lang=<%= lang %>&file=<%= file %>&rep_id=<%= report_id %>&select_door=<%= select_door %>&select_section=<%= select_sect %>&screen=<%= screen %>&amount_data=<%= count_data %>&user_name=<%= ses_user %>" frameborder="0" scrolling="yes"></iframe>
									</div>
								</div>							
							</div>
						</div>					
	...
	<%	}	%>	
		...
		<script>
		function selectBy(){
			location.href = 'report_402_pdf.jsp?file='+document.form1.hiddoor_id.value+'&select_door='+document.form1.select_door.value+'&select_section='+document.form1.select_section.value;
		}

		function changePage(screen){
			location.href = 'report_402_pdf.jsp?file='+document.form1.hiddoor_id.value+'&select_door='+document.form1.select_door.value+'&select_section='+document.form1.select_section.value+'&screen='+screen;
		}
