...
<%	
	...
	session.setAttribute("action", "file_upload_section.jsp?");	
...
<%	
	...
	//	String lastName = fileName.substring(fileName.indexOf(".") +1);
		...
		if((fileName != "") && (lastName.equals("TXT") ||lastName.equals("txt") || lastName.equals("CSV") || lastName.equals("csv"))){
			try{
				rc.WriteDataLogFile("["+ses_user+"] Upload File Section [dbsection]");
				...
				while((message = br.readLine()) != null){
					...
					if(!ckCode || data_arr.length != 5 || data_arr[0].length() > 6 || data_arr[3].length() > 6){
						msgerr.add(message+" -->("+lb_format_file_err+")");
						no_pass++;
					}else if(data_arr[3].equals("")){ //sec_code
							msgerr.add(message+" --> "+msg_notinput_depart);
							no_pass++;
					}else{
						sql = "INSERT INTO dbsection (sec_code, th_desc, en_desc, dep_code) VALUES ('"+data_arr[0]+"','"+data_arr[1]+"','"+data_arr[2]+"','"+data_arr[3]+"')";
						try{
							resultQry = stmtUp.executeUpdate(sql);
							rc.WriteDataLogFile("["+ses_user+"] Insert sec_code : " + data_arr[0] + " [dbsection]");
							pass++;
						}catch(SQLException e){
							if(resultQry != 1){
								sql = "UPDATE dbsection SET th_desc='"+data_arr[1]+"' ,en_desc='"+data_arr[2]+"', dep_code='"+data_arr[3]+"' WHERE (sec_code = '"+data_arr[0]+"')";
								try{
									stmtUp.executeUpdate(sql);
									rc.WriteDataLogFile("["+ses_user+"] Update sec_code : " + data_arr[0] + " [dbsection]");
									pass++;
				...
				rc.WriteDataLogFile("[" + ses_user + "] Record all "+countall+" records, Upload success "+pass+" records, Upload fail "+no_pass+" records [dbsection]");
			}catch(IOException e){
				out.println("<script> ModalDanger_10Second_NoReturn('"+uploadFile_msgerror5+"'); </script>");
			}
		}else{
...
%>		
		<div class="body-display">
			<div class="container" style="margin-top: -20px;">
				...
				<div class="col-md-10">
					<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="file_upload_section.jsp">
						<div class="panel panel-primary"> 
							<div class="panel-heading">
								<h3 class="panel-title"> <b> <%= lb_section %> </b> </h3> 
							</div> 
							<div class="panel-body">
								...
								<div class="table-responsive" style="border: 0px !important;" border="0">
									<div style="min-width: 800px; max-width: 100%; margin-left: 15px; margin-right: 15px;" border="0">
										...
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_departcode %> ( 6 <%= lb_letter2 %> ) </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div> 
