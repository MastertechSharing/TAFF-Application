...
<%	
	...
	session.setAttribute("action", "report_109_pdf.jsp?");
...
<html lang="en">
	<head>
		...
		<script>
			function orderBy(){
				document.form1.action = "report_109_pdf.jsp";
				document.getElementById("form1").submit();
			}
			function changePage(screen){
				document.form1.action = "report_109_pdf.jsp?screen="+screen+"&";
				document.getElementById("form1").submit();
			}
		</script>
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			try{
				...
				stmtUp.executeUpdate(createTableTmpReport(db_database,TempName,"104",db_type));		
			}catch(SQLException e){
			...
			if(!((request.getParameter("select_orderby") == null) || (request.getParameter("select_orderby").equals("")))){
				if(select_orderby.equals("2")){
					sql_orderby = " ORDER BY trs.date_event, trs.idcard, trs.time_event ";
				}
			}
			sql = sql + sql_orderby;
				...
				//	emp_status = rs.getString("reader_duty");	//	Status 1=I, 2=O
					...
					switch(count){
						...
						case 1:		//	Insert Time IN - New IDCard, New Date
							sqlTmp = getSQLInsertTmp104(TempName,rec,idcard,emp_name,emp_date,emp_day,
						...
						case 3: 	//	Insert Time IN - New IDCard
							sqlTmp = getSQLInsertTmp104(TempName,rec,idcard,emp_name,"","",
										emp_time,emp_door,emp_duty,"","","",emp_gdoor);
							break;
						case 9:		//	Not execute
							sqlTmp = "";
								...
								<div class="modal-title col-xs-12 col-md-12">
									...
									<div class="bs-callout bs-callout-info"> 
										<div class="row alert-message-info" style="height: 32px; margin-left: -10px; margin-top: -10px; margin-right: -10px;">
											<label class="col-xs-6 col-md-6 control-label" style="margin-top: 5px;"> <i class="glyphicon glyphicon-list-alt"> </i> &nbsp; <a href="report_109.jsp"> <%= lb_report_109 %> </a> </label>
