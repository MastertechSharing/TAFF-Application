...
<%	
	...
	session.setAttribute("action", "file_upload_group.jsp?");
...
<%										
	...
	//	String lastName = fileName.substring(fileName.indexOf(".") +1);
		...
		if((fileName != "") && (lastName.equals("TXT") ||lastName.equals("txt") || lastName.equals("CSV") || lastName.equals("csv"))){
			try{
				rc.WriteDataLogFile("["+ses_user+"] Upload File Group [dbgroup]");
				...
				while((message = br.readLine()) != null){
					...
					if(!ckCode || data_arr.length != 4 || data_arr[0].length() > 13){
						msgerr.add(message+" -->("+lb_format_file_err+")");
						no_pass++;
					}else{
						sql = "INSERT INTO dbgroup(group_code, th_desc, en_desc) VALUES ('"+data_arr[0]+"','"+data_arr[1]+"','"+data_arr[2]+"')";
						try{
							resultQry = stmtUp.executeUpdate(sql);
							rc.WriteDataLogFile("["+ses_user+"] Update group_code : " + data_arr[0] + " [dbgroup]");
							pass++;
						}catch(SQLException e){
							if(resultQry != 1){
								sql = "UPDATE dbgroup SET th_desc='"+data_arr[1]+"' ,en_desc='"+data_arr[2]+"' WHERE (group_code = '"+data_arr[0]+"')";
								try{
				...
				rc.WriteDataLogFile("[" + ses_user + "] Record all "+countall+" records, Upload success "+pass+" records, Upload fail "+no_pass+" records [dbgroup]");
			}catch(IOException e){
				out.println("<script> ModalDanger_10Second_NoReturn('"+uploadFile_msgerror5+"'); </script>");
			}
		}else{
...
%>
		<div class="body-display">
			<div class="container" style="margin-top: -20px;">
				...
				<div class="col-md-10">
					<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="file_upload_group.jsp">
						<div class="panel panel-primary"> 
							<div class="panel-heading">
								<h3 class="panel-title"> <b> <%= lb_group %> </b> </h3> 
							</div> 
