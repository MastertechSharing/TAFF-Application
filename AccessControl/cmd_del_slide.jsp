...
<%	
	...
	session.setAttribute("subtitle", "delslide");
	session.setAttribute("action", "cmd_del_slide.jsp?");	
		...
		<script>
			...
			$(document).ready(function() {
				...
				$('#btn_ok').on('click', function(){
					...
					onSubmitGetSet(data_selected, document.form1.files, '<%= lb_del_file %>', '<%= lb_delallcmdD2 %>', '<%= lang %>', 'slide', '<%= lb_no_datadoor %>', '<%= msg_ps_selectdoor %>');
					return false;
				});
			});
	...
	<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
			...
			<div class="container">
				...
				<form id="form1" name="form1" method="post">
					...
					<div class="table-responsive" style="border: 0px !important; margin-top: -10px;" border="0">
						<div style="min-width: 800px;" class="table" border="0">
							<div class="bs-callout bs-callout-info" style="margin-left: 5px; margin-right: 5px;"> 
								<div class="row">
									<h5 class="modal-title col-xs-3 col-md-3" style="margin-top: 6px;"> <div align="right"> <b>  <%= lb_selfileslide %> : </b> </div> </h5>
									<div class="modal-title col-xs-3 col-md-3"> 
										<input type="text" class="form-control" id="files" name="files" style="min-height: 32px !important;">
									</div>
									...
									<div class="modal-title col-xs-5 col-md-5"> </div> 
								</div>
								<p>
								<div class="row">
									<h5 class="modal-title col-xs-3 col-md-3"> </h5>
									<h5 class="modal-title col-xs-9 col-md-9"> 
										<input type="checkbox" name="delall" id="delall" onclick="checkedid('<%= lb_msgpicslide_del %>')" /> <%= lb_delallcmdD3 %>
									</h5>
								</div>
							</div>
						</div>
					...
					<div class="col-xs-4" align="center"> 
						...
						<input type="hidden" name="command" value="D3">
		...
		<script>
			function show_detail(){
				view_detail.location = 'iframe_listfile.jsp?listfile=slide';
			...
			function onSubmitGetSet(data_selected, file, sText, sDel, lang, txtPage, msg, msg2){
				var text_warning = "";
				num = 0;
				if("<%= numtaff %>" == 0){
					...
					if(file.value != "" && document.form1.delall.checked == true){
						if(lang == "th"){
							ModalWarning_TextName('กรุณาเลือกลบแบบไฟล์  หรือ ลบไฟล์ทั้งหมด ', '');
						}else{
							ModalWarning_TextName('Select delete file or delete file all', '');
						}
						return false;
					}else{
						if(file.value != ""){
						...
						//	document.getElementById("delall").disabled = true;
							...
							if (!regex.test(fileUpload.value.toLowerCase())) {
								if(lang == "th"){
									ModalWarning_TextName("กรุณาอัปโหลดไฟล์ที่มีนามสกุล  " + allowedFiles + " เท่านั้น", 'file');
								}else{
									ModalWarning_TextName("Please upload files having extensions " + allowedFiles + " only.", 'file');
								}
								return false;
							}else{ 
								if(data_selected == ""){
