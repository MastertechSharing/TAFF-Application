...
<% 
	...
	session.setAttribute("action", "report_201_excel.jsp?");
%>
...
<%
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			String fileName = report_id+"_Transaction_By_Employee.xls";
			int num = 0;
			int num_id = 0;		
			try{
				...
				row_head.createCell((short)7).setCellValue(lb_times); 
				...
				try{
					...
					while (rs.next()) {
						...
						row_query_db.createCell((short)6).setCellValue(rs.getString("reader_no")+" : "+emp_desc);
						row_query_db.createCell((short)7).setCellValue(rs.getString("num"));						
					}
					rs.close();
