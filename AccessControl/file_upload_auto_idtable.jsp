...
<% 
	...
	session.setAttribute("subtitle", "autoidtable");	
	session.setAttribute("action", "file_upload_auto_idtable.jsp?&");
			...
			try{
				...
				while((message = br.readLine()) != null){
					...
					if(!ckCode || data_arr.length != 4){
						...
						no_pass++;
					}else if(data_arr[0].length() > 16 || data_arr[0].length() < 1){ 
						msgerr.add(message+" --> "+alert_idcard+" "+lb_orspace);
						no_pass++;
					}else if(data_arr[1].length() != 1){
						msgerr.add(message+" --> "+alert_status+" "+lb_orspace);
						no_pass++;
					}else if((data_arr[1].length() == 1) && (!(data_arr[1].equals("1") || data_arr[1].equals("2")))){
						msgerr.add(message+" --> "+alert_status2);
						no_pass++;
					}else if((data_arr[2].length() != 10) || (!chkPatternDate(data_arr[2]))){
						msgerr.add(message+" --> ("+alertDT_upload+")");
						no_pass++;
					}else{
						sql = "INSERT INTO proc_auto_idtables(idcard, status_no, auto_date) VALUES ('"+data_arr[0]+"', '"+data_arr[1]+"', '"+dateToYMD(data_arr[2])+"')";
						try{
							resultQry = stmtUp.executeUpdate(sql);
							pass++;
						}catch(SQLException e){
							msgerr.add(message+" --> ("+alert_duprecord+")");
							no_pass++;
						}
					}
					countall++;
				...
				rc.WriteDataLogFile("["+ses_user+"] Upload File Auto IDTable [proc_auto_idtables]");
			}catch(IOException e){
				out.println("<script> ModalDanger_10Second_NoReturn('"+uploadFile_msgerror5+"'); </script>");
			}
		}else{
...
%>		
		<div class="body-display">
			<div class="container" style="margin-top: -20px;">
				...
				<div class="col-md-10">
					<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="file_upload_auto_idtable.jsp">
						<div class="panel panel-primary"> 
							<div class="panel-heading">
								<h3 class="panel-title"> <b> <%= lb_auto_idtable %> </b> </h3> 
							</div> 
								...
								<div class="table-responsive" style="border: 0px !important;" border="0">
									<div style="min-width: 800px; max-width: 100%; margin-left: 15px; margin-right: 15px;" border="0">
										...
										<div class="row">
											...
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_reportstatus %> ( 1 <%= lb_letter2 %> <%= lb_ex2 %> : 1 = <%= lb_adddata %>, 2 = <%= lb_deletedata %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_date %> ( dd/mm/yyyy ) </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
