...
<% 
	...
	session.setAttribute("action", "report_113_excel.jsp?");
%>
		...
		if(check_employee){
			...
			try{
				...
				//	emp_door = rs.getString("door_id")+" : "+rs.getString("door_desc");
					...
					emp_work_code = rs.getString("work_code");
					emp_name = rs.getString("fname")+" "+rs.getString("sname");
					section = rs.getString("sec_code")+" : "+rs.getString("sec_desc");
					rec++;
					if(!(tmp_id.equals(idcard))){
						count_emp++;
						sqlTmp = getSQLInsertTmp113(TempName,rec,idcard,emp_name,emp_date,emp_day,
									emp_time,emp_duty,emp_status,emp_reader,emp_event,emp_work_code);
					}else{
						if(!(tmp_date.equals(emp_date))){
							sqlTmp = getSQLInsertTmp113(TempName,rec,"","",emp_date,emp_day,
										emp_time,emp_duty,emp_status,emp_reader,emp_event,emp_work_code);
						}else{
							sqlTmp = getSQLInsertTmp113(TempName,rec,"","","","",
									emp_time,emp_duty,emp_status,emp_reader,emp_event,emp_work_code);
						}
					}
			...
			String fileName = report_id+"_All_Transaction_Work_Code.xls";
			int num = 0;
			int num_id = 0;		
			try{	
				...
				row_head.createCell((short)8).setCellValue(lb_matchin); 
				row_head.createCell((short)9).setCellValue(lb_reportstatus); 
				row_head.createCell((short)10).setCellValue(lb_event);
				row_head.createCell((short)11).setCellValue(lb_work_code);
				...
				try{
					...
					while(rs.next()){		
						...
						row_query_db.createCell((short)8).setCellValue(emp_status);
						row_query_db.createCell((short)9).setCellValue(emp_duty);
						row_query_db.createCell((short)10).setCellValue(emp_event);
						row_query_db.createCell((short)11).setCellValue(emp_work_code);
					}					
					rs.close();
