...
<%	
	...
	session.setAttribute("subtitle", "setsound");
	session.setAttribute("action", "cmd_set_sound.jsp?");
		...
		<script>
			...
			$(document).ready(function() {
				...
				$('#btn_ok').on('click', function(){
					...
					onSubmitGetSet(data_selected, document.fupload.sound, document.fupload.files, '<%= lang %>', '<%= lb_no_datadoor %>', '<%= msg_ps_selectdoor %>');
					return false;
				});
			});
	...
	<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
			...
			<div class="container">
					...
					<div class="table-responsive" style="border: 0px !important; margin-top: -10px;" border="0">
						<div style="min-width: 800px;" class="table" border="0">
							<div class="bs-callout bs-callout-info" style="margin-left: 5px; margin-right: 5px;"> 
								<div class="row">
									<h5 class="modal-title col-xs-3 col-md-3" style="margin-top: 6px;"> <div align="right"> <b> <%= lb_fileupload %> : </b> </div> </h5>
									<div class="modal-title col-xs-6 col-md-6" style="margin-top: 6px;"> 
										<input type="file" name="files" id="files" class="filestyle" data-buttonBefore="true" data-buttonText="&nbsp; Choose File .mp3" data-buttonName="btn-primary" data-size="xs" onChange="checkInputFile('<%= lb_detailsond %>')"  accept=".mp3">
									</div>
								...
								<p>
								<div class="row">
									<div class="modal-title col-xs-3 col-md-3"> </div>
									<div class="modal-title col-xs-9 col-md-9"> <font color="red"> *** <%= lb_detailsond %> </font> </div>
								</div>
								<p>
								<div class="row">
									<h5 class="modal-title col-xs-3 col-md-3"> <div align="right"> <b> <%= lb_typesound %> : </b> </div> </h5>
									<div class="modal-title col-xs-2 col-md-2"> <%= lb_thai %> </div>
									<div class="modal-title col-xs-7 col-md-7" style="margin-left: -80px;"> 
										<input type="radio" name="sound" value="0" checked> <%= lb_passth %> &nbsp; &nbsp; &nbsp; 
										<input type="radio" name="sound" value="2"> <%= lb_nopassth %> &nbsp; &nbsp; &nbsp; 
										<input type="radio" name="sound" value="4"> <%= lb_passinth %> &nbsp; &nbsp; &nbsp; 
										<input type="radio" name="sound" value="6"> <%= lb_passoutth %> &nbsp; &nbsp; &nbsp; 
									</div>
								</div>
								<p>
								<div class="row">
									<h5 class="modal-title col-xs-3 col-md-3"> </h5>
									<div class="modal-title col-xs-2 col-md-2"> <%= lb_eng %> </div>
									<div class="modal-title col-xs-7 col-md-7" style="margin-left: -80px;"> 
										<input type="radio" name="sound" value="1"> <%= lb_passen %> &nbsp; &nbsp; &nbsp; 
										<input type="radio" name="sound" value="3"> <%= lb_nopassen %> &nbsp; &nbsp; &nbsp; 
										<input type="radio" name="sound" value="5"> <%= lb_passinen %> &nbsp; &nbsp; &nbsp; 
										<input type="radio" name="sound" value="7"> <%= lb_passouten %> &nbsp; &nbsp; &nbsp; 
									</div>
								</div>
							</div>
						</div>
				...
				<form id="form1" name="form1" method="post">
					...
					<div class="col-xs-4" align="center"> 
						...
						<input type="hidden" name="command" value="66">
						<input type="hidden" name="filename">
						<input type="text" name="param" id="param" value="1" style="display: none;">
						<input type="hidden" name="typesound" id="typesound">
		...
		<script>
			...
			function checkInputFile(text){
				...
				//	Checking file type
				var re_text = /\.mp3|\.MP3/i;
				var filename = path.value;	
				if(filename != null){			
					if(filename.search(re_text) != -1){
					//	document.fupload.action = 'module/upload_file.jsp?type=sound&typesound='+value_s;
			...
			function onSubmitGetSet(data_selected, rdo, file, lang, msg, msg2){
				var r1 = '';
				var value_s = '';
				...
				if("<%= numtaff %>" == 0){
					...
					if(document.fupload.files.value == ""){
						if(lang == "th"){
							ModalWarning_TextName('กรุณาเลือกไฟล์เสียงที่จะอัพโหลด','');
						}else{
							ModalWarning_TextName('Please select file sound for upload','');
						}
						return false;
					}else{
