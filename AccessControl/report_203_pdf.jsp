...
<% 
	...
	session.setAttribute("action", "report_203_pdf.jsp?");
...
<html lang="en">
	<head>
		...
		<script>
			function orderBy(){
				document.form1.action = "report_203_pdf.jsp";
				document.getElementById("form1").submit();
			}
			function changePage(screen){
				document.form1.action = "report_203_pdf.jsp?screen="+screen+"&";
				document.getElementById("form1").submit();
			}
		</script>
...
<%	
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			String sql = "SELECT COUNT(*) AS count_rec, trs.idcard, trs.reader_no, SUBSTRING(trs.reader_no, 1, 4) AS reader_door, ";
			if(mode == 0){
			...
			sql = sql + "GROUP BY trs.idcard, trs.date_event, trs.reader_no, rd.th_desc, rd.en_desc, emp.th_fname, emp.th_sname , emp.en_fname, emp.en_sname, emp.sec_code, sec.th_desc, sec.en_desc ";
			
			String select_orderby = request.getParameter("select_orderby");
			String sql_orderby = "ORDER BY emp.sec_code, trs.idcard, trs.date_event, trs.reader_no ";   
			if(!((request.getParameter("select_orderby") == null) || (request.getParameter("select_orderby").equals("")) || (request.getParameter("select_orderby").equals("null")))){
				if(select_orderby.equals("2")){
					sql_orderby = "ORDER BY emp.sec_code, trs.date_event, trs.idcard, trs.reader_no ";
				}else if(select_orderby.equals("3")){
					sql_orderby = "ORDER BY emp.sec_code, trs.reader_no, trs.idcard, trs.date_event ";
				}
			}
			sql = sql + sql_orderby;
			...
			try{
				...
				while(rs.next()){
					...
					emp_door = rs.getString("reader_door");
					emp_reader = rs.getString("reader_no");
					emp_num = rs.getInt("count_rec");
					emp_desc = rs.getString("rd_desc");
					emp_name = rs.getString("fname")+" "+rs.getString("sname");
					rec++;
					if(!(tmp_id.equals(idcard))){
						count_emp++;
						sqlTmp = getSQLInsertTmp201(TempName,rec,idcard,emp_name,emp_date,emp_day,emp_reader,emp_desc,emp_num);
						stmtUp.executeUpdate(sqlTmp);
					}else{
						if(!(tmp_date.equals(emp_date))){
							sqlTmp = getSQLInsertTmp201(TempName,rec,"","",emp_date,emp_day,emp_reader,emp_desc,emp_num);
							stmtUp.executeUpdate(sqlTmp);
						}else{
							if(!(tmp_door.equals(emp_door))){
								sqlTmp = getSQLInsertTmp201(TempName,rec,"","","","",emp_reader,emp_desc,emp_num);
								stmtUp.executeUpdate(sqlTmp);
							}
						}
					}
								...
								<div class="modal-title col-xs-12 col-md-12">
									...
									<div class="bs-callout bs-callout-info"> 
										<div class="row alert-message-info" style="height: 32px; margin-left: -10px; margin-top: -10px; margin-right: -10px;">
											<label class="col-xs-6 col-md-6 control-label" style="margin-top: 5px;"> <i class="glyphicon glyphicon-list-alt"> </i> &nbsp; <a href="report_203.jsp"> <%= lb_report_203 %> </a> </label>
									...
									%>
											...
											<div class="col-xs-2 col-md-2"> 
												<select class="form-control <%= selectpicker %>" data-width="100%" data-size="15" data-container="body" name="select_orderby" style="max-height: 28px !important; font-size: 12px !important;" onChange="orderBy()">
													...
													<option value="1" <% if(select_orderby.equals("1")){ %> selected <%}%>> <%= lb_code %> </option>
													<option value="2" <% if(select_orderby.equals("2")){ %> selected <%}%>> <%= lb_date %> </option>
													<option value="3" <% if(select_orderby.equals("3")){ %> selected <%}%>> <%= lb_reader %> </option>
												</select>
											</div>
							...
							<div class="modal-title col-xs-12 col-md-12">
								...
								<div class="bs-callout bs-callout-info"> 
									<div class="row" align="center">
										<iframe name="show_body" width="97%" height="700" src="ReportSumByPerson.do?st_date=<%= date1 %>&ed_date=<%= date2 %>&time1=<%= time1 %>&time2=<%= time2 %>&emp_id=<%= emp_id %>&select_section=<%= section %>&start=<%= startRow %>&rep_id=<%= report_id %>&lang=<%= lang %>" frameborder="0" scrolling="yes"></iframe>
									</div>
								</div>
