	...
	<head>
		...
		<script>
			function LoadDataTable(onload){
				...
				$(document).ready(function() {
					...
					//	"stateSave": true,
						"bServerSide": true,
						"bProcessing": true,
						"sAjaxSource": "file_download_txt_iframe_server_side.jsp?data=<%= path.replace("\\", "?")%>",
					...
					//	console.log(cRow_file);
					//	console.log(rows_selected_file.join(','));
					//	console.log(rows_selected_file);
					/*	
						var sumsize = 0;
						var split_size = rows_selected_file.join(',').split(',');
						for(var i = 0; i <= rows_selected_file.join(',').split(',').length; i++){
							console.log(split_size[i].split('?'));
							
						//	sumsize = Number(sumsize) + Number((split_size[i].split('?'))[1]);
						}
					*/
						if(cRow_file != 0){
							downloadMultiFile();
			...
			//	location.href = 'file_download_txt_download.jsp?action=single&path='+$("#path").val()+'&file_type='+$("#file_type").val()+'&filename='+filename;
				...
				$('#form1').attr('action', 'file_download_txt_download.jsp?action=single_&path='+$("#path").val()+'&file_type='+$("#file_type").val()+'&filename='+filename);
				$('#form1').attr('target', 'iframe_download');
				$('#form1').submit();
				
				$('#data_file').DataTable().ajax.reload( null, false );
			...
			//	$('#btn_multi').prop('disabled', true);
				...
				$('#form1').attr('action', 'file_download_txt_download.jsp?action=multi_&path='+$("#path").val()+'&file_type='+$("#file_type").val()+'&listfile='+$("#check_file").val());
				$('#form1').attr('target', 'iframe_download');
				$('#form1').submit();
			...
			<div style="max-width: 800px;" class="table" border="0">
				...
				<div class="row" style="margin-left: 0px; margin-right: 0px;">
					<div class="col-xs-12 col-md-12">
						...
						<div class="bs-callout bs-callout-info" style="margin-right: -10px;"> 
							...
							<div class="row">
								<div class="col-xs-12 col-md-12">
									<div style="text-align: right; margin-top: 0px; margin-left: 0px; margin-right: 0px;">
							...
							<div class="row">
								<div class="col-xs-12 col-md-12">
									<div class="row" style="margin-top: -30px; margin-bottom: -4px;">
										<table class="table table-bordered table-hover" id="data_file" name="data_file" align="center" border="0" style="max-width: 99% !important; margin-bottom: -5px;">
											<thead>
												<tr>
										...
										%>
												<tr>
													...
													<td align="center" style="max-width: 100px;"> 
														<a href="#" onClick="downloadSingleFile('<%= fileNames[i] %>');"> <i class="glyphicon glyphicon-download-alt" data-toggle="tooltip" data-placement="left" title="<%= lb_download %>"> </i> </a> 
													</td>
												</tr>
											<%     		}
													}
										...
										<input type="button" class="btn btn-primary btn-sm max-height-26 button-shadow1 button-shadow2" name="btn_multi" id="btn_multi" style="margin-left: 8px;" value=" <%= lb_download_filezip %> " onMouseOver="this.style.cursor='hand';" data-toggle="tooltip" data-placement="right" title="<%= lb_when_download_txt %>"> </input>
