...
<% 
	...
	session.setAttribute("action", "file_upload_blacklist.jsp?&");
...
<%	
	...
	//	String lastName = fileName.substring(fileName.indexOf(".") +1);
		...
		if((fileName != "") && (lastName.equals("TXT") ||lastName.equals("txt") || lastName.equals("CSV") || lastName.equals("csv"))){
			try{
				rc.WriteDataLogFile("["+ses_user+"] Upload File Blacklist [dbblacklist]");
				...
				/* 	if(message.length() > 376){
						message = message.substring(0, 376);
					}
				 */	data_arr = (message+",#").split(",");
					
					boolean chk_pass = true;
					boolean ckCode = chkPatternCode(data_arr[0].trim());
					if(!ckCode || data_arr.length != 6){
						msgerr.add(message+" --> ("+lb_format_file_err+")");
						no_pass++;	chk_pass = false;
					}else if(data_arr[0].length() > 16){
						msgerr.add(message+" --> ("+alert_idcard_over16+")");
						no_pass++;	chk_pass = false;
					}else if(data_arr[1].length() > 80){
						msgerr.add(message+" --> ("+alert_fullname_over80+")");
						no_pass++;	chk_pass = false;
					}else if((data_arr[2].trim().length() == 0)){
						msgerr.add(message+" --> ("+alert_detail_equals0+")" );
						no_pass++;	chk_pass = false;
					}else if((data_arr[2].length() > 250)){
						msgerr.add(message+" --> ("+alert_detail_over250+")" );
						no_pass++;	chk_pass = false;
					}else{

						if((data_arr[3].trim().length() == 0)){
							data_arr[3] = dateToYMD(getCurrentDate());
						}else{
							if(!chkPatternDate(data_arr[3])){
								msgerr.add(message+" --> ("+alertDT_upload+")");
								no_pass++;	chk_pass = false;
							}
						}
						if((data_arr[4].trim().length() == 0)){
							data_arr[4] = ses_user;
						}
						...
						if(chk_pass == true){
							try{
								String set_cardid = "";
								if(data_arr[0].trim().length() != 13){
									sql = " INSERT INTO dbblacklist (idcard, fullname, record_detail, record_date, record_by) "
									+ " VALUES('"+data_arr[0].toUpperCase()+"', '"+data_arr[1]+"', '"+data_arr[2]+"', '"+data_arr[3]+"', '"+data_arr[4]+"') ";
								}else{
									sql = " INSERT INTO dbblacklist (idcard, fullname, record_detail, record_date, record_by, card_id) "
									+ " VALUES('"+data_arr[0].toUpperCase()+"', '"+data_arr[1]+"', '"+data_arr[2]+"', '"+data_arr[3]+"', '"+data_arr[4]+"', '"+data_arr[0].trim()+"') ";
								}
								resultQry = stmtUp.executeUpdate(sql);
								if(resultQry != 0){
									rc.WriteDataLogFile("[" + ses_user + "] Blacklist idcard : " + data_arr[0] + " [dbblacklist]");
									pass++;
								}
							}catch(SQLException eupdate){
							//	msgerr.add(message+" --> ("+sql+")");
							//	msgerr.add(message+" --> ("+eupdate.getMessage()+")");
								msgerr.add(message+" --> ("+lb_date+" "+data_arr[3]+" "+msg_dupblacklist3+" "+data_arr[0].toUpperCase()+" "+msg_dupblacklist2+")" );
								no_pass++;
							}
						}
					}
				...
				rc.WriteDataLogFile("[" + ses_user + "] Record all "+countall+" records, Upload success "+pass+" records, Upload fail "+no_pass+" records [dbblacklist]");
			}catch(IOException e){
				out.println("<script> ModalDanger_10Second_NoReturn('"+uploadFile_msgerror5+"'); </script>");
			}
		}else{
...
%>		
		<div class="body-display">
			<div class="container" style="margin-top: -20px;">
				...
				<div class="col-md-10">
					<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="file_upload_blacklist.jsp">
						<div class="panel panel-primary"> 
							<div class="panel-heading">
								<h3 class="panel-title"> <b> <%= lb_blacklist %> </b> </h3> 
							</div> 
								...
								<div class="table-responsive" style="border: 0px !important;" border="0">
									<div style="min-width: 800px; max-width: 100%; margin-left: 15px; margin-right: 15px;" border="0">
										...
										<div class="row">
											...
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_names %> ( 80 <%= lb_letter %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_blacklist_detail %> ( 250 <%= lb_letter %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_blacklist_date %> ( dd/mm/yyyy <%= lb_orspace %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_blacklist_by %> ( 16 <%= lb_letter %> <%= lb_orspace %> ) </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
