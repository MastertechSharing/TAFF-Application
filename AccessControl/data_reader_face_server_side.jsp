...
<%	
	...
	if (session.getAttribute("ses_permission") == null) {
		...
		if(view_data.equals("all")){
			setWhere = " WHERE (door.hardware_model = 'VF1000') and ( door.duplicate_ip != '"+view_data+"' ) ";
		}else{
			setWhere = " WHERE (door.hardware_model = 'VF1000') and ( door.duplicate_ip = '"+view_data+"' ) ";
		}
		
		int total = getCountRecord("dbdoor", "hardware_model", "VF1000", stmtQry);
		...
		try{
			...
			while (rs.next()) {			
				...
				JSONArray ja = new JSONArray();
				if(checkPermission(ses_per, "0")){
					ja.put("<center> <a href='edit_reader_face.jsp?action=edit&door_id="+door_id+"'> <img src='images/edit.png' width='18' height='18' border='0' align='absmiddle' data-toggle='tooltip' data-placement='right' data-html='true' title='"+lb_editdata+"'> </a> </center>");
