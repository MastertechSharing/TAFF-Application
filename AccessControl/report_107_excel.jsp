...
<% 
	...
	session.setAttribute("action", "report_107_excel.jsp?");
%>
...
<%
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			String time_last = "";
			ResultSet rs = null;	
			// เพื่อจะเอาเวลาเข้า-ออกจริงๆของบริษัทมาคำนวนการมาสายออกก่อน ของพนักงาน		
			try{
				...
				out.println("<div class='alert alert-danger' role='alert'> SQL Exception2 :"+e2.getMessage()+"</div>");			
			}
			...
			try{
				...
				while(rs.next()){
					...
					door1 = rs.getString("min_dt").substring(20,24);
					if(rs.getString("min_dt").equals(rs.getString("max_dt"))){
						time_out = "";
						door2 = "";
					}else{
						time_out = rs.getString("max_dt").substring(11,19);	
						door2 = rs.getString("max_dt").substring(20,24);
					}
					emp_name = rs.getString("fname")+" "+rs.getString("sname");
					section = rs.getString("sec_code")+" : "+rs.getString("sec_desc");				
					show_time_in = compareTimeToString(time_first,time_in);
					show_time_out = compareTimeToString(time_out,time_last);
					rec++;
					if(!(tmp_id.equals(idcard))){
						count_emp++;
						sqlTmp = getSQLInsertTmp107(TempName,rec,idcard,emp_name,emp_date,emp_day,time_in,show_time_in,door1,time_out,show_time_out,door2);
					}else{
						sqlTmp = getSQLInsertTmp107(TempName,rec,"","",emp_date,emp_day,time_in,show_time_in,door1,time_out,show_time_out,door2);
					}
			...
			String fileName = report_id+"_Time_Late.xls"; 	
			int num = 0;
			int num_id = 0;
			try{ 
				...
				row_head.createCell((short)7).setCellValue(lb_timeinlate);  
				row_head.createCell((short)8).setCellValue(lb_doors); 
				row_head.createCell((short)9).setCellValue(lb_timeout); 
				row_head.createCell((short)10).setCellValue(lb_timeoutlate);  
				row_head.createCell((short)11).setCellValue(lb_doors); 
				row_head.createCell((short)12).setCellValue(lb_comment);
				...
				try{
					...
					while (rs.next()) {				
						...
						row_query_db.createCell((short)7).setCellValue(rs.getString("time_in_late"));
						row_query_db.createCell((short)8).setCellValue(rs.getString("reader_in"));
						row_query_db.createCell((short)9).setCellValue(rs.getString("time_out"));
						row_query_db.createCell((short)10).setCellValue(rs.getString("time_out_late"));
						row_query_db.createCell((short)11).setCellValue(rs.getString("reader_out"));
						row_query_db.createCell((short)12).setCellValue("");			
					}			
					rs.close();	
