...
<% 
	...
	session.setAttribute("action", "report_114_excel.jsp?");
%>
...
<%	
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			if(lang.equals("th")){
				sql = sql + "rd.th_desc AS reader_desc, ev.th_desc AS ev_desc, emp.th_fname AS fname, emp.th_sname AS sname, sec.th_desc AS sec_desc, ";
			}else{
				sql = sql + "rd.en_desc AS reader_desc, ev.en_desc AS ev_desc, emp.en_fname AS fname, emp.en_sname AS sname, sec.en_desc AS sec_desc, ";
			}
			sql = sql + "rd.reader_no, emp.sec_code "
			...
			if(!((request.getParameter("select_orderby") == null) || (request.getParameter("select_orderby").equals("")) || (request.getParameter("select_orderby").equals("null")))){
				if(select_orderby.equals("2")){
					sql_orderby = "ORDER BY emp.sec_code, trs.date_event, trs.time_event, trs.idcard, trs.reader_no ";
				}else if(select_orderby.equals("3")){
					sql_orderby = "ORDER BY emp.sec_code, trs.reader_no, trs.idcard, trs.date_event, trs.time_event ";
				}if(select_orderby.equals("4")){
					sql_orderby = "ORDER BY emp.sec_code, trs.event_code, trs.idcard, trs.date_event, trs.time_event ";
				}
			}
			sql = sql + sql_orderby;
			...
			try{
				...
				//	emp_door = rs.getString("door_id")+" : "+rs.getString("door_desc");
					emp_reader = rs.getString("reader_no")+" : "+rs.getString("reader_desc");
					emp_event = rs.getString("event_code")+" : "+rs.getString("ev_desc");
					emp_temperature = rs.getString("temperature");
					emp_mask = rs.getString("wearmask");
					emp_name = rs.getString("fname")+" "+rs.getString("sname");
					section = rs.getString("sec_code")+" : "+rs.getString("sec_desc");
					rec++;
					if(!(tmp_id.equals(idcard))){
						count_emp++;
						sqlTmp = getSQLInsertTmp114(TempName,rec,idcard,emp_name,emp_date,emp_day,
									emp_time,emp_duty,emp_reader,emp_event,emp_temperature,emp_mask);
					}else{
						if(!(tmp_date.equals(emp_date))){
							sqlTmp = getSQLInsertTmp114(TempName,rec,"","",emp_date,emp_day,
										emp_time,emp_duty,emp_reader,emp_event,emp_temperature,emp_mask);
						}else{
							sqlTmp = getSQLInsertTmp114(TempName,rec,"","","","",
									emp_time,emp_duty,emp_reader,emp_event,emp_temperature,emp_mask);
						}
					}
			...
			String fileName = report_id+"_All_Transaction_temperature.xls";
			int num = 0;
			int num_id = 0;		
			try{	
				...
				HSSFCell cell = row_compa.createCell((short)4);			
				cell.setCellValue(companyName);		
				HSSFCell cell2 = row_rep.createCell((short)0);
				cell2.setCellValue("[Rep:"+report_id+"]");		
				HSSFCell cell3 = row_rep.createCell((short)4);
				...
				row_head.createCell((short)6).setCellValue(lb_time);  
				row_head.createCell((short)7).setCellValue(lb_readers); 				
				row_head.createCell((short)8).setCellValue(lb_reportstatus); 
				row_head.createCell((short)9).setCellValue(lb_event);
				row_head.createCell((short)10).setCellValue(lb_temperature);
				row_head.createCell((short)11).setCellValue(lb_wearmask);
				...
				try{
					...
					while(rs.next()){		
						...
						emp_reader = rs.getString("reader_desc");
						emp_event = rs.getString("event_desc");
						if (emp_name.length() > 30) {
							emp_name = emp_name.substring(0, 29);
						}
						if (emp_reader.length() > 30) {
							emp_reader = emp_reader.substring(0, 29);
						}
						if (rs.getString("wearmask").equals("1")) {
							emp_mask = lb_wear;
						} else {
							emp_mask = lb_notwear;
						}
						...
						row_query_db.createCell((short)6).setCellValue(emp_time);
						row_query_db.createCell((short)7).setCellValue(emp_reader);						
						row_query_db.createCell((short)8).setCellValue(emp_duty);
						row_query_db.createCell((short)9).setCellValue(emp_event);
						row_query_db.createCell((short)10).setCellValue(emp_temperature);
						row_query_db.createCell((short)11).setCellValue(emp_mask);
					}					
					rs.close();
