...
<% 	
	...
	session.setAttribute("action", "data_mapduty.jsp?action="+getaction+"&");
...
<html lang="en">
	<head>
		...
		<script language="javascript">
			...
			function ModalConfirm(sCode, sText, duty_out, th_name, en_name){
				document.getElementById("text_confirm").innerHTML = sText;
				document.getElementById("text_code").innerHTML = sCode;
				document.getElementById("text_duty_out").innerHTML = duty_out;
			...
			function Confirm_Button(){
				location.href = 'module/act_mapduty.jsp?action=del&duty_in='+document.getElementById('sCode').value;
			}
	...
	<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onLoad="Clear_Session_Table('<%= clear_session %>', 'DataTables_data_table_<%= request.getRequestURI() %>'); Load_DataTable();">
				...
				<div class="table-responsive" style="border: 0px !important;" border="0">
					...
					<div style="min-width: 800px;" class="table" border="0">	
						...
						<table style="min-width: 800px;" align="center" id="data_table" class="table table-striped table-bordered" cellspacing="0" >
							...
							<%	if(checkPermission(ses_per, "0")){	%>	
									...
									<td width="3%" align="center"> </td>
							<%	}	%>
									<td width="17%" align="center"> <%= lb_duty_in %> </td>
									<td width="17%" align="center"> <%= lb_duty_out %> </td>
							...
							<%	
								try{
									ResultSet rs = stmtQry.executeQuery("SELECT * FROM dbmapduty ORDER BY duty_in ASC ");
									while(rs.next()){								
										String duty_in = rs.getString("duty_in");
							%>
								<tr>
							<%	if(checkPermission(ses_per, "0")){	%>			
									<td align="center"> 
										<a href="edit_mapduty.jsp?action=edit&duty_in=<%= duty_in %>"> <img src="images/edit.png" width="18" height="18" border="0" align="absmiddle" data-toggle="tooltip" data-placement="right" title="<%= lb_editdata %>"> </a>
									</td>
									<td align="center"> 
										<img src="images/delete.png" width="20" height="20" border="0" align="absmiddle" onClick="ModalConfirm('<%= duty_in %>', '<%= msg_confirmdel %>', '<%= rs.getString("duty_out") %>', '<%= rs.getString("th_desc") %>', '<%= rs.getString("en_desc") %>');" data-toggle="tooltip" data-placement="right" title="<%= lb_deletedata %>">
									</td>
							<%	}	%>		
									<td align="center"> <b> <a href="#" onClick="show_detail('<%= duty_in %>');" data-toggle="tooltip" data-placement="right" data-html="true" title="<%= lb_viewdata %>"> <%= duty_in %> </a> </b> </td>
									<!--<td align="center"> <b> <a href="#" onClick="show_detail('<%= rs.getString("duty_out") %>');" data-toggle="tooltip" data-placement="right" data-html="true" title="<%= lb_viewdata %>"> <%= rs.getString("duty_out") %> </a> </b> </td>
									-->
									<td class="pad-left-10"> <%= rs.getString("duty_out") %> </td>
					...
					<div class="col-xs-12" align="left" style="margin-left: -15px; min-width: 500px;">
						...
						<%@ include file="tools/toggle_dropdown.jsp"%>
							<%	if(checkPermission(ses_per, "0")){	%>	
							<button type="button" class="btn btn-primary btn-sm" onClick="Add_Data('edit_mapduty.jsp?action=add');"> 
							...
							<div class="col-xs-12 col-md-12" style="margin-bottom: 15px;">
								<strong>
									<div class="alert alert-warning" role="alert" style="min-width: 50px;">
										<div class="row">
											<div class="col-xs-5 col-md-5" align="right"> <%= lb_duty_in %> :  </div>
											<div class="col-xs-7 col-md-7" align="left"> <p id="text_code"> </p> </div>
										</div>
										<div class="row">
											<div class="col-xs-5 col-md-5" align="right"> <%= lb_duty_out %> :  </div>
											<div class="col-xs-7 col-md-7" align="left"> <p id="text_duty_out"> </p> </div>
										</div>
										<div class="row">
		...
		<script>
		function show_detail(duty_in){
			view_detail_140px.location = 'view_mapduty.jsp?action=view&duty_in='+duty_in;
