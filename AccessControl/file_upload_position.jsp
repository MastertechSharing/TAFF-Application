...
<%	
	...
	session.setAttribute("action", "file_upload_position.jsp?");
...
<%	
	...
	//	String lastName = fileName.substring(fileName.indexOf(".") +1);
		...
		if((fileName != "") && (lastName.equals("TXT") ||lastName.equals("txt") || lastName.equals("CSV") || lastName.equals("csv"))){
			try{
				rc.WriteDataLogFile("["+ses_user+"] Upload File Position [dbposition]");
				...
				while((message = br.readLine()) != null){
					...
					if(!ckCode || data_arr.length != 4 || data_arr[0].length() > 6){
						msgerr.add(message+" -->("+lb_format_file_err+")"+data_arr.length+data_arr[0].length()+ckCode);
						no_pass++;
					}else{
						sql = "INSERT INTO dbposition (pos_code, th_desc, en_desc) VALUES ('"+data_arr[0]+"','"+data_arr[1]+"','"+data_arr[2]+"')";
						try{
							resultQry = stmtUp.executeUpdate(sql);
							rc.WriteDataLogFile("["+ses_user+"] Insert pos_code : " + data_arr[0] + " [dbposition]");
							pass++;
						}catch(SQLException e){
							if(resultQry != 1){
								sql = "UPDATE dbposition SET th_desc='"+data_arr[1]+"',en_desc='"+data_arr[2]+"' WHERE (pos_code = '"+data_arr[0]+"')";
								try{
									stmtUp.executeUpdate(sql);
									rc.WriteDataLogFile("["+ses_user+"] Update pos_code : " + data_arr[0] + " [dbposition]");
									pass++;
				...
				rc.WriteDataLogFile("[" + ses_user + "] Record all "+countall+" records, Upload success "+pass+" records, Upload fail "+no_pass+" records [dbposition]");
			}catch(IOException e){
				out.println("<script> ModalDanger_10Second_NoReturn('"+uploadFile_msgerror5+"'); </script>");
			}
		}else{
...
%>	
		<div class="body-display">
			<div class="container" style="margin-top: -20px;">
				...
				<div class="col-md-10">
					<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="file_upload_position.jsp">
						<div class="panel panel-primary"> 
							<div class="panel-heading">
								<h3 class="panel-title"> <b> <%= lb_position %> </b> </h3> 
							</div> 
