...
<%	
	...
	try{
		...
		String pathfile = "";
		if(!ip.equals("")){
			pathfile = path_data+ip+"\\ListVideo.txt";
		}else{
			pathfile = path_data+door_id+"\\ListVideo.txt";
		}
		File f = new File(pathfile);
		if(!f.exists()){
			out.println("<div class='alert alert-danger' role='alert' style='max-height: 32px;'>");
			out.println("	<div class='alert-link' style='margin-top: -10px;'> "+msg_not_file_upload+" "+lb_cmd12+" "+lb_doorcode+" : "+door_id+" - "+desc+" </div>");
			out.println("</div>");
		}else{
%>
				<div class="alert alert-info" role="alert" style="max-height: 32px;">
					<div class="alert-link" style="margin-top: -10px;"> <%= lb_cmd12 %> <%= lb_doorcode %> : <%= door_id %> - <%= desc %> </div>
				</div>
