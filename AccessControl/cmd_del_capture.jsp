...
<%	
	...
	session.setAttribute("subtitle", "delcapture");
	session.setAttribute("action", "cmd_del_capture.jsp?");
...
<html lang="en">
		...
		<script>
			...
			function oncheckAll(){
				if(document.getElementById('chkDel').checked == true){
					...
					document.getElementById('end_date').style.background = "f0f0f0";
				}else{
					document.getElementById('st_date').style.background = "ffffff";
					document.getElementById('end_date').style.background = "ffffff";
				}
			}
	...
	<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onLoad="oncheckAll();">
			...
			<div class="container">
				...
				<form id="form1" name="form1" method="post">
						...
						<div style="min-width: 800px;" class="table" border="0">
							<div class="bs-callout bs-callout-info" style="margin-left: 5px; margin-right: 5px;"> 
								<div class="row">
									...
									<h5 class="modal-title col-xs-2 col-md-2" style="margin-top: 5px;"> <div align="right"> <b> <%= lb_todate %> : </b> </div> </h5>
									<div class="modal-title col-xs-2 col-md-2"> 
										<div class="input-group date form_date" data-date="" data-date-format="dd/mm/yyyy" data-link-field="end_date" data-link-format="dd/mm/yyyy">
											<input class="form-control" type="text" value="<%= getCurrentDate() %>" style="max-height: 30px;" readonly>
											<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
										</div>
										<input type="hidden" class="form-control" id="end_date" name="end_date" maxlength="10" value="<%= getCurrentDate() %>" readonly="readonly" style="background-color:#F0F0F0">
									</div>
								...
								<div class="row">
									...
									<div class="modal-title col-xs-9 col-md-9"> 
										<input type="checkbox" name="chkDel" id="chkDel" onClick="oncheckAll()" > &nbsp; <%= lb_delallcmdD0 %>
										<input type="hidden" name="hidDate" value="<%= getCurrentDate()%>">
									</div>
								</div>
							</div>
						</div>
					...
					<div class="col-xs-4" align="center"> 
						...
						<input type="hidden" name="command" value="D0">
		...
		<script>					
			...
			function onSubmitGetSet(data_selected, lang, msg, msg2){
				if(lang == "th"){
					sDel = 'ลบภาพถ่ายทั้งหมด?';
				}else {
					sDel = 'Delete Capture Picture All?';
				}
				...
				if("<%= numtaff %>" == 0){
					...
					if(document.getElementById('chkDel').checked == true){
						if(data_selected == ""){
							...
							document.form1.filename.value = '0';
							if(lang == 'th'){
								document.getElementById('text_confirm').innerHTML = "ลบภาพถ่ายทั้งหมด ?";
							}else{
								document.getElementById('text_confirm').innerHTML = "Delete Capture Picture All ?";
							}
						...
						if(numdate > 31 || numdate2 > 31){
							if(lang == 'th'){
								ModalWarning_TextName("กำหนดค่าวันที่ไม่เกิน 31 วัน", "");
							}else{
								ModalWarning_TextName("Set date not over 31 days", "");
							}
							return false;
						}else{
							if(data_selected == ""){
								...
								document.form1.filename.value = '1';
