						...
						if(data.trim().length() != 0){
							...
							if(data.length() == 50){
								...
								if(!data_error){
									...
									message_taf = convData.ConvertDataTAF(data);
									if(message_taf == 1){			//	1 = Success
										countRecOK++;
									}else{
										countRecFail++;
										data_error = true;
										if(message_taf == 0){		//	0 = Fail Format Error
											fail_message = uploadFile_msgerror6;
										}else if(message_taf == 2){		//	2 = Software Event 6 Uncheck
											fail_message = lb_events + " " + data.substring(40, 42) + " " + lb_software6_uncheck;
										}else if(message_taf == 3){		//	3 = No File config.set or path
											fail_message = uploadFile_msgerror7;
										}else if(message_taf == 4){		//	4 = No Set Standard File
		...
		document.onreadystatechange = function () {
			if (document.readyState === "complete") {
				setTimeout(function(){
					...
					parent.document.getElementById("btn_taf").disabled = false;
				...
				//	parent.document.getElementById('space').style.display = "";
					...
					setTimeout(function(){
						window.top.$('#process_res').text('<%= lb_show_process_to_taf %>');
