...
<%
	...
	if (action.equals("clear")) {
		try{
			resultQry = stmtUp.executeUpdate("UPDATE dblock SET time1='', time2='', time3='', "
					+ "time4='', time5='' WHERE (day_type = '" + day_type + "')");
			if (resultQry != 0) {
				ckActionCmd = "2";
				session.setAttribute("session_alert", msg_cleartimecomplete);
				rc.WriteDataLogFile("[" + ses_user + "] Clear day_type : " + day_type + " [dblock]");
				out.println("<script>document.location='../data_lock.jsp';</script>");
			}
		...
		if (action.equals("edit")) {
			try{
				resultQry = stmtUp.executeUpdate("UPDATE dblock SET day_type='" + day_type + "', time1='" + time1
				...
				if (resultQry != 0) {
					...
					rc.WriteDataLogFile("[" + ses_user + "] Edit day_type : " + day_type + " [dblock]");
					out.println("<script>document.location='../data_lock.jsp';</script>");
				}
			}catch (SQLException e){
				out.println("<div class='alert alert-danger' role='alert'> SQL Exception :"+e.getMessage()+"</div>");
	...
	if (!(ckActionCmd.equals("0"))) {
		...
		while (rs.next()) {
			...
			try{
				stmtUp.executeUpdate(insertDbStatus(doorID, "35", ipAddr, ckActionCmd, date_updates));
			}catch (SQLException e1){
				stmtUp.executeUpdate(updateDbStatus(doorID, "35", ipAddr, ckActionCmd, date_updates));
			}
		}
		rs.close();
	}
