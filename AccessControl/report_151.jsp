										...
										<div class="row">
											...
											<div class="modal-title col-xs-3 col-md-3"> 
												<select class="form-control" name="select_orderby" id="select_orderby" style="max-height: 28px !important; font-size: 12px !important;">
													...
													<option value="1"> <%= lb_empcode %> </option>
									...
									<div class="bs-callout bs-callout-info"> 
										...
										<div class="row" style="margin-top: -25px;">
											...
											<table class="table table-bordered table-hover" id="data_reader" align="center" border="0" style="max-width: <%= table_reader_max_width %> !important; margin-bottom: -5px;">
												<thead>
													<tr>
														<th width="<%= col_reader_checkbox %>" style="text-align: center;"> </th>
														<th width="<%= col_reader_desc %>" style="text-align: center;"> <% if(lang.equals("th")){ out.println(lb_thdesc); }else{ out.println(lb_endesc); } %> </th>
														<th width="<%= col_reader_group %>" style="text-align: center;"> <%= lb_groupdoor %> </th>
													</tr>
												</thead>
												<tbody>
											<%	int numdoor = 0, numsection = 0;
												try{
													...
													ResultSet rs_taff = stmtQry.executeQuery(" SELECT r.*, d.group_door FROM dbreader r LEFT OUTER JOIN dbdoor d ON r.door_id = d.door_id WHERE d.group_door != '' "+where_reader_list+" ORDER BY r.reader_no ASC ");
													while(rs_taff.next()){
														numdoor++;
											%>		
													<tr>
														<td align="center"> <%= rs_taff.getString("r.reader_no") %> </td>
														<td class="pad-left-10"> 
															<div class="ellipsis_string" style="max-width: <%= set_ellipsis_max_width %>;">
															<%	out.print("<b><span onClick='show_reader(\""+rs_taff.getString("r.reader_no")+"\");' style='color: #337AB7; cursor: pointer; cursor: hand;' data-toggle='tooltip' data-placement='right' title='"+lb_viewdata+"'>" + rs_taff.getString("r.reader_no") + "</span></b> -");	%>
															<%	if(lang.equals("th")){ out.print(rs_taff.getString("th_desc")); }else{ out.print(rs_taff.getString("en_desc")); } %>
															</div>
														</td>
														<td align="center"> <%= rs_taff.getString("d.group_door") %> </td>
													</tr>
											<%	
													}	rs_taff.close();
												}catch(SQLException e){
		...
		<script>
			...
			function fncSubmit(lang, msg, msg2, msg3, msg4, cRow_door, cRow_section, supervisor){
					...
					if(document.form1.emp_id.value == ""){
						...
						if(cRow_section != ''){
							if(lang == "th"){
								...
								ModalWarning_TextName("You selected section. Can not select idcard", "");
							}
				...
				if(document.form1.pagefile.value == "pdf"){		
					...
					document.form1.action = "report_151_excel.jsp";
				}
