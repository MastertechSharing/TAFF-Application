...
<%	
	...
	session.setAttribute("subtitle", "setvideo");
	session.setAttribute("action", "cmd_set_video.jsp?");
		...
		<script>
			...
			$(document).ready(function() {
				...
				$('#btn_ok').on('click', function(){
					...
					onSubmitGetSet(data_selected, document.fupload.files, '<%= lang %>', '<%= lb_no_datadoor %>', '<%= msg_ps_selectdoor %>');
					return false;
				});
			});
	...
	<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
			...
			<div class="container">
							...
							<div class="bs-callout bs-callout-info" style="margin-left: 5px; margin-right: 5px;"> 
								<div class="row">
									...
									<div class="modal-title col-xs-6 col-md-6" style="margin-top: 6px;"> 
										<input type="file" name="files" id="files" class="filestyle" data-buttonBefore="true" data-buttonText="&nbsp; Choose File .mp4" data-buttonName="btn-primary" data-size="xs" onChange="checkInputFile('<%= lb_msg_mp4 %>')" multiple accept="video/mp4">
									</div>
								...
								<div class="row">
									...
									<div class="modal-title col-xs-9 col-md-9"> <font color="red"> *** <%= lb_msg_mp4 %> </font> </div>
								</div>
							</div>
						</div>
					</div>
				...
				<form id="form1" name="form1" method="post">
					...
					<div class="col-xs-4" align="center"> 
						...
						<input type="hidden" name="command" value="64">
		...
		<script>
			...
			function checkInputFile(text){
				...
				//	Checking file type
				var re_text = /\.mp4|\.MP4/i;
				var filename = path.value;		
				if(filename != null){			
				   if(filename.search(re_text) != -1){	
						document.fupload.action = 'module/upload_file.jsp?type=video';
			...
			function onSubmitGetSet(data_selected, file, lang, msg, msg2){
				if("<%= numtaff %>" == 0){
					...
					if(document.fupload.files.value == ""){
						if(lang == "th"){
							ModalWarning_TextName('กรุณาเลือกไฟล์วิดีโอที่จะอัพโหลด','');
						}else{
							ModalWarning_TextName('Please select file video for upload','');
						}
						return false;
