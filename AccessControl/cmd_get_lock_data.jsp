...
<%	
	...
	session.setAttribute("subtitle", "getlock");
	String ip = "", door_id = "";
	...
	session.setAttribute("action", "cmd_get_lock_data.jsp?ip="+ip+"&door_id="+door_id+"&");
%>
<span/>
<html lang="en">
	<head>
...
<%	
	...
	try{
		...
		String pathfile = "";
		if(!ip.equals("")){
			pathfile = path_data+ip+"\\LOCK.CFG";
		}else{
			pathfile = path_data+door_id+"\\LOCK.CFG";
		}
		File f = new File(pathfile);
		if(!f.exists()){
			out.println("<div class='alert alert-danger' role='alert'>");
			out.println("	<span class='alert-link'> "+msg_not_file_upload+" "+lb_getlock+" "+lb_doorcode+" : "+door_id+" - "+desc+" </span>");
			out.println("</div>");
		}else{
%>
				<div class="alert alert-info" role="alert">
					<span class="alert-link"> <%= lb_getlock %> <%= lb_doorcode %> : <%= door_id %> - <%= desc %> </span>
				</div>
				
				<table style="min-width: 800px;" class="table table-hover" align="center" id="table1" border="0">
					<thead>
						<tr>
							<td width="25%" align="center"> <label class="control-label"> <%= lb_day %> </label> </td>
							<%	for(int i = 1; i <= 5; i++){	%>
...
<%					
			...
			String textStr = "";
			String dayType = "", time1 = "", time2 = "", time3 = "", time4 = "", time5 = "";
