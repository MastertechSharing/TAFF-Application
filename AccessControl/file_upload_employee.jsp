...
<%	
	...
	session.setAttribute("action", "file_upload_employee.jsp?");
				...
				while((message = br.readLine()) != null){					
					...
					if(rec_no == 1 || message.trim().length() == 0){
						...
						if(!ckCode || data_arr.length != 29 || data_arr[0].length() > 16 || data_arr[1].length() > 1 || data_arr[2].length() > 1 || data_arr[22].length() > 1){
							...
							no_pass++;
						}else if((data_arr[23].length() != 5) || data_arr[24].length() != 5){
							msgerr.add(message+" --> ("+alertT_upload+")" );
							no_pass++;
						}else{
							if(!chkPatternDate(data_arr[9]) || !chkPatternDate(data_arr[10])){
								msgerr.add(message+" --> ("+alertDT_upload+")");
								no_pass++;
							}else if(!chkPatternTime2(data_arr[23]) || !chkPatternTime2(data_arr[24])){
								msgerr.add(message+" --> ("+alertT_upload+")");
								no_pass++;
							}else if(data_arr[11].length() > 6 || data_arr[11].equals("")){ //sec_code
								msgerr.add(message+" --> "+alert_codes+" "+lb_orspace);
								no_pass++;
							}else if(data_arr[12].length() > 6 || data_arr[12].equals("")){ //pos_code
								msgerr.add(message+" --> "+alert_codespos+" "+lb_orspace);
								no_pass++;
							}else if(data_arr[13].length() > 13 || data_arr[13].equals("")){ //group_code
								msgerr.add(message+" --> "+alert_codesgroup+" "+lb_orspace);
								no_pass++;
							}else if(data_arr[14].length() > 6 || data_arr[14].equals("")){ //type_code
								msgerr.add(message+" --> "+alert_codestype+" "+lb_orspace);
								no_pass++;
							}else{
								issue = data_arr[7].trim();		//check_issue(data_arr[7]);
								pincode = data_arr[8].trim();	//check_pincode(data_arr[8]);
								st_date = dateToYMD(data_arr[9]);
								ex_date = dateToYMD(data_arr[10]);
								sec_code = data_arr[11];
								pos_code = data_arr[12];
								group_code = data_arr[13];
								type_code = data_arr[14];
								sn_card = data_arr[16].trim();							
								cardid = data_arr[19].trim();
								phone = data_arr[20].trim();
								email = data_arr[21].trim();
								use_finger = data_arr[22].trim();
								...
								if(!(phone.equals(""))){
									if(phone.length() >= 10 || phone.length() <= 15){
										if (!chkPatternPhone(phone)){
											phone = "";
											msgerr.add(message+" --> "+alert_format_phone);
										}
									}else{
										phone = "";
									}
								...
								if(!(email.equals(""))){
									if (!chkPatternEmail(email)){
										email = "";
										msgerr.add(message+" --> "+alert_emails);
									}
								}
...
%>
		<div class="body-display">
			<div class="container" style="margin-top: -20px;">
				...
				<div class="col-md-10">
					<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="file_upload_employee.jsp">
						<div class="panel panel-primary"> 
							<div class="panel-heading">
								<h3 class="panel-title"> <b> <%= lb_employee %> </b> </h3> 
							</div> 
							<div class="panel-body"> 
								...
								<div class="table-responsive" style="border: 0px !important;" border="0">
									<div style="min-width: 800px; max-width: 100%; margin-left: 15px; margin-right: 15px;" border="0">
										...
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_thname %> ( 100 <%= lb_letter2 %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_thsname %> ( 100 <%= lb_letter2 %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_enname %> ( 100 <%= lb_letter2 %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_ensname %> ( 100 <%= lb_letter2 %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
										...
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_empcard %> ( 16 <%= lb_letter %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
										...
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_nationality %> ( 20 <%= lb_letter %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_cardid %> ( 13 <%= lb_letter %> <%= lb_orspace %> <%= lb_ex2 %> : XXXXXXXXXXXXX ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_phoneno %> ( 15 <%= lb_letter %> <%= lb_orspace %> <%= lb_ex2 %> : XXXXXXXXXX <%= lb_or2 %> XXX-XXX-XXXX <%= lb_or2 %> XXX-XXXXXXX ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_email %> ( 50 <%= lb_letter2 %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
							...
							<%	if(msgerr.size() != 0){	%>
								...
								<div class="table-responsive" style="border: 0px !important;" border="0">
									<div style="min-width: 750px; max-width: 100%; margin-top: 15px; margin-left: 15px; margin-right: 15px;" border="0">
