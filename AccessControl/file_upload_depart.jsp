...
<% 
	...
	session.setAttribute("action", "file_upload_depart.jsp?&");
...
<%	
	...
	//	String lastName = fileName.substring(fileName.indexOf(".") +1);
		...
		if((fileName != "") && (lastName.equals("TXT") ||lastName.equals("txt") || lastName.equals("CSV") || lastName.equals("csv"))){
			try{
				rc.WriteDataLogFile("["+ses_user+"] Upload File Depart [dbdepart]");
				...
				while((message = br.readLine()) != null){
					...
					if(!ckCode || data_arr.length != 4 || data_arr[0].length() > 6){
						...
						sql = "INSERT INTO dbdepart(dep_code, th_desc, en_desc) VALUES ('"+data_arr[0]+"','"+data_arr[1]+"','"+data_arr[2]+"')";
						try{
							resultQry = stmtUp.executeUpdate(sql);
							rc.WriteDataLogFile("[" + ses_user + "] Insert dep_code : " + data_arr[0] + " [dbdepart]");
							pass++;
						}catch(SQLException e){
							if(resultQry != 1){
								sql = "UPDATE dbdepart SET th_desc='"+data_arr[1]+"',en_desc='"+data_arr[2]+"' WHERE (dep_code = '"+data_arr[0]+"')";
								try{
									stmtUp.executeUpdate(sql);
									rc.WriteDataLogFile("[" + ses_user + "] Update dep_code : " + data_arr[0] + " [dbdepart]");
									pass++;
				...
				rc.WriteDataLogFile("[" + ses_user + "] Record all "+countall+" records, Upload success "+pass+" records, Upload fail "+no_pass+" records [dbdepart]");
			}catch(IOException e){
				out.println("<script> ModalDanger_10Second_NoReturn('"+uploadFile_msgerror5+"'); </script>");
			}
		}else{
...
%>		
		<div class="body-display">
			<div class="container" style="margin-top: -20px;">
				...
				<div class="col-md-10">
					<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="file_upload_depart.jsp">
						<div class="panel panel-primary"> 
							<div class="panel-heading">
								<h3 class="panel-title"> <b> <%= lb_depart %> </b> </h3> 
							</div> 
								...
								<div class="table-responsive" style="border: 0px !important;" border="0">
									<div style="min-width: 800px; max-width: 100%; margin-left: 15px; margin-right: 15px;" border="0">
										...
										<div class="row">
											...
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_departcode %> ( 6 <%= lb_letter %> ), </label> 
										</div>
										<div class="row">
