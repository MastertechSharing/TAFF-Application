...
<%@ page import="server.util.DatetimeUtil"%>
...
<% 
	...
	session.setAttribute("action", "report_115_excel.jsp?");
%>
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			try{
				...
				stmtUp.executeUpdate(createTableTmpReport(db_database,TempName,"115",mode));		
			}catch(SQLException e){
			...
			try{
				...
				//	emp_status = rs.getString("reader_duty");	//	Status 1=I, 2=O
					...
					if(!(tmp_id.equals(idcard))){
						if(emp_duty.equals("I") || emp_duty.equals("i")){
							count = 0;
						}else if(emp_duty.equals("O") || emp_duty.equals("o")){
							count = 1;
						}
						count_emp++;
					}else{
						...
						if(!(tmp_date.equals(emp_date))){
							...
							if(tmp_door.equals(emp_door)){
								if(emp_duty.equals("I") || emp_duty.equals("i")){
									...
									if(tmp_duty.equals("I") || tmp_duty.equals("i")){
										count = 6;	//	update
									}else{
										count = 5;
									}
					...
					switch(count){
						case 0:
							sqlTmp = getSQLInsertTmp115(TempName,rec,idcard,emp_name,emp_date,
						...
						case 1:
							...
							break;
						case 2:
							sqlTmp = getSQLInsertTmp115(TempName,rec,"","",emp_date,emp_day,
						...
						case 3:
							...
							break;
						case 4:
							sqlTmp = getSQLInsertTmp115(TempName,rec,"","","","",
										emp_time,emp_door,emp_duty,"","","",emp_gdoor);
							break;
						case 5:
							sqlTmp = getSQLInsertTmp115(TempName,rec,"","","","",
							...
							break;
						case 6:
							sqlTmp = getSQLUpdateTmp115(TempName,rec,emp_time,emp_door,emp_gdoor,emp_duty);
							break;
					}
			...
			String fileName = report_id+"_In-Out_Transaction_Calculate_Time.xls";		
			int num = 0;
			int num_id = 0;		
			try{
				...
				row_head.createCell((short)10).setCellValue(lb_breakTime); 							
				...
				try{
					...
					while (rs.next()) {
						...
						String startDate = "";
						String endDate = "";
						DateFormat formatDateTimeCheck = DatetimeUtil.formatTime;
						if (!rs.getString("date_work").equals("")) {
							startDate = rs.getString("date_work") + " ";
							endDate = rs.getString("date_work") + " ";
							formatDateTimeCheck = DatetimeUtil.formatDateTime;
						}
						if (!rs.getString("time_in").equals("") && !rs.getString("time_out").equals("")) {
							startDate += rs.getString("time_in");
							endDate += rs.getString("time_out");
							breakTime = DatetimeUtil.checkDateTimeDiffToFormatTime(startDate, endDate, formatDateTimeCheck);
						}
											
						row_query_db.createCell((short)10).setCellValue(breakTime);
					}
					rs.close();
