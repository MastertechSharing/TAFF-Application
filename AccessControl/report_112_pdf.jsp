...
<%	
	...
	session.setAttribute("action", "report_112_pdf.jsp?");
...
<html lang="en">
	<head>
		...
		<script>
			function orderBy(){
				document.form1.action = "report_112_pdf.jsp";
				document.getElementById("form1").submit();
			}
			function changePage(screen){
				document.form1.action = "report_112_pdf.jsp?screen="+screen+"&";
				document.getElementById("form1").submit();
			}
		</script>
...
<%		
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			String sql = "SELECT trs.*, SUBSTRING(trs.reader_no, 5, 1) AS reader_duty, ";								
			if(mode == 0){
			...
			if(lang.equals("th")){
				sql = sql + "door.th_desc AS door_desc, ev.th_desc AS ev_desc, emp.th_fname AS fname, emp.th_sname AS sname, sec.th_desc AS sec_desc, ";
			}else{
				sql = sql + "door.en_desc AS door_desc, ev.en_desc AS ev_desc, emp.en_fname AS fname, emp.en_sname AS sname, sec.en_desc AS sec_desc, ";
			}
			sql = sql + "door.door_id, emp.sec_code, emp.sec_code "
			...
			String sql_orderby = "ORDER BY trs.idcard, trs.date_event, trs.time_event, trs.reader_no ";   
			if(!((request.getParameter("select_orderby") == null) || (request.getParameter("select_orderby").equals("")) || (request.getParameter("select_orderby").equals("null")))){
				if(select_orderby.equals("2")){
					sql_orderby = "ORDER BY emp.sec_code, trs.idcard, trs.date_event, trs.time_event, trs.reader_no ";
				}else if(select_orderby.equals("3")){
					sql_orderby = "ORDER BY trs.date_event, trs.time_event, emp.sec_code, trs.idcard, trs.reader_no ";
				}else if(select_orderby.equals("4")){
					sql_orderby = "ORDER BY trs.reader_no, trs.date_event, trs.time_event, emp.sec_code, trs.idcard ";
				}else if(select_orderby.equals("5")){
					sql_orderby = "ORDER BY trs.event_code, trs.date_event, trs.time_event, emp.sec_code, trs.idcard ";
				}
			}
			sql = sql + sql_orderby;
			...
			try{
				...
				while(rs.next()){
					...
					emp_sec = rs.getString("sec_code")+" : "+rs.getString("sec_desc");
					emp_date = rs.getString("date_work");
					emp_day = rs.getString("day_work");
					emp_duty = rs.getString("duty");
					emp_time = rs.getString("time_event");
					emp_status = rs.getString("reader_duty");
					emp_reader = rs.getString("reader_no");
					emp_event = rs.getString("event_code")+" : "+rs.getString("ev_desc");
					rec++;
					if(!(tmp_id.equals(idcard))){
						count_emp++;
						sqlTmp = getSQLInsertTmp112(TempName,rec,idcard,emp_name,emp_sec,emp_date,emp_day,
									emp_time,emp_duty,emp_status,emp_reader,emp_event);
					}else{
						if(!(tmp_sec.equals(emp_sec))){
							sqlTmp = getSQLInsertTmp112(TempName,rec,"","",emp_sec,emp_date,emp_day,
										emp_time,emp_duty,emp_status,emp_reader,emp_event);
						}else{
							if(!(tmp_date.equals(emp_date))){
								sqlTmp = getSQLInsertTmp112(TempName,rec,"","","",emp_date,emp_day,
										emp_time,emp_duty,emp_status,emp_reader,emp_event);
							}else{
								sqlTmp = getSQLInsertTmp112(TempName,rec,"","","","","",
										emp_time,emp_duty,emp_status,emp_reader,emp_event);
							}
						}
					}
			...
			try{
				String sql_c = "SELECT count(DISTINCT id_card) as countrec FROM " + TempName + " WHERE id_card != '' ";
				ResultSet rs_c = stmtTmp.executeQuery(sql_c);
				if(rs_c.next()){
					totalRow = rs_c.getInt("countrec");
				}	rs_c.close();
			}catch(SQLException e){	
				out.println("<div class='alert alert-danger' role='alert'> SQL Exception :"+e.getMessage()+"</div>");	
			}
			
			totalPage = (int)Math.ceil((double)getCountRecord(TempName,stmtQry)/(double)rowOfPage);				
%>			
								...
								<div class="modal-title col-xs-12 col-md-12">
									...
									<div class="bs-callout bs-callout-info"> 
										<div class="row alert-message-info" style="height: 32px; margin-left: -10px; margin-top: -10px; margin-right: -10px;">
											<label class="col-xs-6 col-md-6 control-label" style="margin-top: 5px;"> <i class="glyphicon glyphicon-list-alt"> </i> &nbsp; <a href="report_112.jsp"> <%= lb_report_112 %> </a> </label>
											<label class="col-xs-4 col-md-4" style="margin-top: 5px; text-align: right;"> 
												<%= lb_alldata %> <%= nf1.format(totalRow) %> <%= lb_records %> &nbsp;
											</label>
										...
										<p>
										<div class="row">
											<div class="col-xs-7 col-md-7"> <input type="hidden" name="select_section" id="elect_section" value=""> </div>
									<%	if((emp_id == null) || (emp_id.equals("")) || (ses_per != 9)){	%>
											...
											<div class="col-xs-2 col-md-2"> 
												<select class="form-control <%= selectpicker %>" data-width="100%" data-size="15" data-container="body" name="select_orderby" style="max-height: 28px !important; font-size: 12px !important;" onChange="orderBy()">
													...
													<option value="2" <% if(select_orderby.equals("2")){ %> selected <%}%>> <%= lb_sect %> </option>
													<option value="3" <% if(select_orderby.equals("3")){ %> selected <%}%>> <%= lb_date %> </option>
													<option value="4" <% if(select_orderby.equals("4")){ %> selected <%}%>> <%= lb_doorcode %> </option>
													<option value="5" <% if(select_orderby.equals("5")){ %> selected <%}%>> <%= lb_eventcode %> </option>
												</select>
											</div>
							...
							<div class="modal-title col-xs-12 col-md-12">
								...
								<div class="bs-callout bs-callout-info"> 
									<div class="row" align="center">
										<iframe name="show_body" width="97%" height="700" src="ReportTransAllInternalExternalPerson.do?st_date=<%= date1 %>&ed_date=<%= date2 %>&time1=<%= time1 %>&time2=<%= time2 %>&emp_id=<%= emp_id %>&select_section=<%= section %>&start=<%= startRow %>&rep_id=<%= report_id %>&lang=<%= lang %>" frameborder="0" scrolling="yes"></iframe>
									</div>
								</div>
