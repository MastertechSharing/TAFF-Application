...
<% 
	...
	session.setAttribute("action", "report_102_excel.jsp?");
%>
...
<%	
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		if(check_employee){
			...
			String fileName = report_id+"_All_Transaction_Internal_And_External_Person.xls";
			int num = 0;
			int num_id = 0;		
			try{	
				...
				row_section.createCell((short)7).setCellValue(lb_date+"   "+getCurrentDate()+"   "+lb_time+"  "+getCurrentTimeShort());	
				...
				row_head.createCell((short)7).setCellValue("(RD-Duty)");
				row_head.createCell((short)8).setCellValue(lb_event); 			
				...
				try{
					...
					while(rs.next()){		
						...
						row_query_db.createCell((short)7).setCellValue(" (" + emp_reader+"-"+emp_duty + ")");
						row_query_db.createCell((short)8).setCellValue(emp_event);				   
					}					
					rs.close();
