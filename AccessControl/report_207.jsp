...
<% 
	...
	session.setAttribute("subtitle", "report_207");
	session.setAttribute("action", "report_207.jsp?");
...
<html lang="en">
		...
		<script>
			...
			$(document).ready(function() {
				...
				$('#btn_action button').on('click', function(){
					...
					fncSubmit('<%= lang %>', '<%= lb_no_datareader %>', '<%= msg_ps_selectreader %>', cRow_door);
					return false;
				});
	...
	<%	}	%>	
		...
		<script>
			...
			function fncSubmit(lang, msg, msg2, cRow_door){
				...
				if(document.form1.pagefile.value == "pdf"){		
					document.form1.action = "report_207_pdf.jsp";
				}else if(document.form1.pagefile.value == "xls"){
					document.form1.action = "report_207_excel.jsp";
				}
