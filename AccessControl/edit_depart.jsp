...
<% 
	...
	session.setAttribute("subtitle", "depart"); 
	String action = "";
	if(request.getParameter("action") != null){
		action = request.getParameter("action");
	}else{
		response.sendRedirect("data_depart.jsp");
	}
	session.setAttribute("action", "edit_depart.jsp?"+"&action="+action+"&dep_code="+request.getParameter("dep_code")+"&");		
			...
			function change_class(field_name){
				if(field_name == 'dep_code'){
					if(document.form1.dep_code.value != ""){
						document.getElementById("textbox_dep_code").className = "form-group has-success has-feedback";
						document.getElementById("icon_dep_code").className = "glyphicon glyphicon-ok form-control-feedback";
					}else{
						document.getElementById("textbox_dep_code").className = "form-group has-error has-feedback";
						document.getElementById("icon_dep_code").className = "glyphicon glyphicon glyphicon-star form-control-feedback";
					}
				}
			}
		</script>
...
<% 		if(action.equals("add")){	%> 
			<div class="row form-group">
				...
				<div class="col-md-4">
					<div class="form-group has-error has-feedback" id="textbox_dep_code" style="margin-bottom: 0px">
						<input type="text" class="form-control" id="dep_code" name="dep_code" maxlength="6" placeholder="<%= lb_departcode %>" onKeyPress="IsValidCharacter()" onKeyUp="change_class('dep_code');" onBlur="change_class('dep_code');">
			...
			<div class="row form-group" style="margin-bottom: 0px;">
				<div class="col-md-12" align="center"> 
					<input type="button" class="btn btn-primary btn-sm button-shadow1 button-shadow2" id="btnok" value=" &nbsp; &nbsp; &nbsp; <%= btn_ok %> &nbsp; &nbsp; &nbsp; " onClick="return Confirm_add_edit(document.form1.dep_code, '<%= msg_confirmedit %>', '<%= msg_notinput_depart %>', 'module/act_depart.jsp?action=add', 'add');"> &nbsp; 
					<input type="button" class="btn btn-default btn-sm button-shadow1 button-shadow2" value=" &nbsp; &nbsp; &nbsp; <%= btn_cancel %> &nbsp; &nbsp; &nbsp; " onClick="location.href='data_depart.jsp'">
				</div>
			</div> 
...
<% 		}else if(action.equals("edit")){			
			...
			try{
				ResultSet rs = stmtQry.executeQuery("SELECT * FROM dbdepart WHERE (dep_code = '"+dep_code+"') ");
				while(rs.next()){				
%>
				<div class="row form-group">
					<label class="control-label label-text-1 col-md-2"> <%= lb_departcode %> : </label>
					<div class="col-md-4">
						<div class="form-group has-success has-feedback" id="textbox_dep_code" style="margin-bottom: 0px">
							<input type="text" class="form-control" id="dep_code" name="dep_code" value="<%= dep_code %>" maxlength="6" placeholder="<%= dep_code %>" onKeyPress="IsValidCharacter()" onKeyUp="change_class('dep_code');" onBlur="change_class('dep_code');">
							<span class="glyphicon glyphicon-ok form-control-feedback" id="icon_dep_code" aria-hidden="true"> </span>
							<input type="hidden" name="dep_code2" id="dep_code2" value="<%= dep_code %>">
						</div>
					</div>
					<div class="col-md-6" style="margin-top: 6px;"> </div>
				</div>
				...
				<div class="row form-group" style="margin-bottom: 0px;">
					<div class="col-md-12" align="center"> 
						<input type="button" class="btn btn-primary btn-sm button-shadow1 button-shadow2" id="btnok" value=" &nbsp; &nbsp; &nbsp; <%= btn_ok %> &nbsp; &nbsp; &nbsp; " onClick="return Confirm_add_edit(document.form1.dep_code, '<%= msg_confirmedit %>', '<%= msg_notinput_depart %>', 'module/act_depart.jsp?action=edit', 'edit');"> &nbsp; 
						<input type="button" class="btn btn-default btn-sm button-shadow1 button-shadow2" value=" &nbsp; &nbsp; &nbsp; <%= btn_cancel %> &nbsp; &nbsp; &nbsp; " onClick="location.href='data_depart.jsp'">
					</div>
				</div> 						
<%	
					...
					<div class="modal-body" align="center">
						<div class="table-responsive" style="border: 0px !important;" border="0"> 
							...
							<div class="col-xs-12 col-md-12" style="margin-bottom: 15px;"> 
								...
								<div class="col-xs-5 col-md-5">
									<button type="button" class="btn btn-default btn-sm button-shadow1 button-shadow2" onClick="javascript: location.href = 'data_depart.jsp';" style="width: 100%;"> <%= btn_cancel %> </button>
								</div>
