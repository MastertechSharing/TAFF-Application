...
<% 
	String idcard = "", fullname = "", group_code = "";
	if(!(request.getParameter("idcard") == null || request.getParameter("idcard").equals("") || request.getParameter("idcard").equals("null"))){			
		idcard = request.getParameter("idcard");
		try{
			ResultSet rs = stmtQry.executeQuery("SELECT th_fname, th_sname, en_fname, en_sname, group_code FROM dbemployee WHERE (idcard = '"+idcard+"')");
			rs.next();
			if(lang.equals("th")){
	...
	session.setAttribute("iframe_emp", "1");
		...
		<form id="form1" name="form1" method="post">
			<div class="table-responsive" style="border: 0px !important; margin-top: -10px;" border="0">
				<div style="min-width: 800px;" class="table" border="0">
							...
							<div class="modal-title col-xs-1 col-md-1"> 
								...
								<%	
									if(getModuleF(stmtQry).equals("0")){
										out.println("<input type='hidden' name='command' value='80'>");
									}else{
										out.println("<input type='hidden' name='command' value='44'>");
									}
								%>
					...
					<%	if(sw_version.equals("67")){	%>
						<input type="hidden" name="typebio" id="radio3" value="A">	
					<%	}else{	%>
						<p>
						<div class="row" style="margin-top: -10px; margin-bottom: -10px;">
							<h5 class="modal-title col-xs-3 col-md-3" style="margin-top: -10px;"> <div align="right"> <b> <%= lb_readertype %> : </b> </div> </h5>
							<h5 class="modal-title col-xs-9 col-md-9" style="margin-top: -10px;"> 
								<div style="min-height: 24px;">
									<input name="typebio" id="radio3" type="radio" value="A" style="margin-left:  0px;"> &nbsp; Finger
									<input name="typebio" id="radio4" type="radio" value="B" style="margin-left: 30px;"> &nbsp; Palm
									<input name="typebio" id="radio5" type="radio" value="1" style="margin-left: 30px;" checked> &nbsp; <%= lb_all %>
								</div>
							</h5>
						</div>
					<%	}	%>
			...
			<div class="table-responsive" style="border: 0px !important; margin-top: -10px; margin-bottom: 0px;" border="0">
				...
				<div style="min-width: 800px; <% if(sw_version.equals("67")){ %> max-height: 350px; <% }else{ %> max-height: 330px; <% } %> overflow-y: none;" class="table" border="0">
				...
				<%	
					...
					try{
						String sql = selectDbDoorByHostByGroupCode(group_code, ses_per, ses_group_user, ses_control_reader);
