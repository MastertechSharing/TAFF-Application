...
<% 	
	...
	session.setAttribute("action", "data_depart.jsp?action="+getaction+"&");
	...
	<head>
		...
		<script language="javascript">
			...
			function Confirm_Button(){
				location.href = 'module/act_depart.jsp?action=del&dep_code='+document.getElementById('sCode').value;
			}
				...
				<div class="table-responsive" style="border: 0px !important;" border="0">
					...
					<div style="min-width: 800px;" class="table" border="0">	
						...
						<table style="min-width: 800px;" align="center" id="data_table" class="table table-striped table-bordered" cellspacing="0" >
							...
							<%	if(checkPermission(ses_per, "013")){	%>	
									...
									<td width="3%" align="center"> </td>
							<%	}	%>
									<td width="24%" align="center"> <%= lb_departcode %> </td>
							...
							<%	
								try{
									...
									if(checkPermission(ses_per, "034")){
										sql = "SELECT * FROM dbdepart ORDER BY dep_code ASC";
									}else{
										sql = "SELECT dep.* FROM dbdepart dep "
											+ "LEFT JOIN dbusers u ON u.dep_code = dep.dep_code "
											+ "WHERE u.user_name = '"+ses_user+"' AND u.dep_code = dep.dep_code "
							...
							<%	if(checkPermission(ses_per, "013")){	%>			
									<td align="center"> 
										<a href="edit_depart.jsp?action=edit&dep_code=<%= dep_code %>"> <img src="images/edit.png" width="18" height="18" border="0" align="absmiddle" data-toggle="tooltip" data-placement="right" title="<%= lb_editdata %>"> </a>
									</td>
									<td align="center"> 
										<img src="images/delete.png" width="20" height="20" border="0" align="absmiddle" onClick="ModalConfirm('<%= dep_code %>', '<%= msg_confirmdel %>', '<%= rs.getString("th_desc") %>', '<%= rs.getString("en_desc") %>');" data-toggle="tooltip" data-placement="right" title="<%= lb_deletedata %>">
									</td>
							<%	}	%>		
									<td align="center"> <b> <a href="#" onClick="show_detail('<%= dep_code %>');" data-toggle="tooltip" data-placement="right" data-html="true" title="<%= lb_viewdata %>"> <%= dep_code %> </a> </b> </td>
					...
					<div class="col-xs-12" align="left" style="margin-left: -15px; min-width: 500px;">
						<div class="btn-group">
							...
							<%	if(checkPermission(ses_per, "013")){	%>	
							<button type="button" class="btn btn-primary btn-sm" onClick="Add_Data('edit_depart.jsp?action=add');"> 
