...
<% 	
	...
	session.setAttribute("action", "data_door.jsp?action="+getaction+"&");
...
<html lang="en">
	<head>
		...
		<script>
			...
			function Load_DataTable(data){
				$(document).ready(function() {
					$('#data_table').dataTable( {
						...
						"bServerSide": true,
						"bProcessing": true,
						"sAjaxSource": "data_door_server_side.jsp?view_data="+data,
			...
			function Confirm_Button(){ 
				location.href = 'module/act_door.jsp?action=del&door_id='+document.getElementById('sCode').value;
			}
	...
	<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" onLoad="Clear_Session_Table('<%= clear_session %>', 'DataTables_data_table_<%= request.getRequestURI() %>'); Load_DataTable('<%= view_data %>');">
				...
				<div class="table-responsive" style="border: 0px !important;" border="0">
					...
					<div class="col-xs-12" align="left" style="margin-left: -15px; min-width: 500px;">
						<div class="btn-group">
							...
							<%	if(checkPermission(ses_per, "0")){	%>	
							<button type="button" class="btn btn-primary btn-sm" onClick="Add_Data('edit_door.jsp?action=add');"> 
		...
		//	Load_DataTable(data);
			$('#data_table').DataTable().ajax.url('data_door_server_side.jsp?view_data='+data_view).load();
		}
