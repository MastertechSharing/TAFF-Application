...
<% 
	...
	session.setAttribute("action", "report_206_pdf.jsp?");
...
<html lang="en">
	<head>
		...
		<script>
			function orderBy(){
				document.form1.action = "report_206_pdf.jsp";
				document.getElementById("form1").submit();
			}
			function changePage(screen){
				document.form1.action = "report_206_pdf.jsp?screen="+screen+"&";
				document.getElementById("form1").submit();
			}
		</script>
...
<%	
	...
	if(date_time1.compareTo(date_time2) > 0){
		...
		try{
			...
			stmtUp.executeUpdate(createTableTmpReport(db_database,TempName,"202",db_type));		
		}catch(SQLException e){
		...
		sql = sql + "door.door_id "
				...
				+ "AND (door.door_id is not null) "	//	"AND (trs.event_code <> '47') ";
		...
		if(!(emp_id == null || emp_id.equals("") || emp_id.equals("null"))){
			sql = sql + "AND (trs.event_code = '"+emp_id+"') ";				
		}
								...
								<div class="modal-title col-xs-12 col-md-12">
									...
									<div class="bs-callout bs-callout-info"> 
										<div class="row alert-message-info" style="height: 32px; margin-left: -10px; margin-top: -10px; margin-right: -10px;">
											<label class="col-xs-6 col-md-6 control-label" style="margin-top: 5px;"> <i class="glyphicon glyphicon-list-alt"> </i> &nbsp; <a href="report_206.jsp"> <%= lb_report_206 %> </a> </label>
							...
							<div class="modal-title col-xs-12 col-md-12">
								...
								<div class="bs-callout bs-callout-info"> 
									<div class="row" align="center">
										<iframe name="show_body" width="97%" height="700" src="ReportSumByDoor.do?st_date=<%= date1 %>&ed_date=<%= date2 %>&time1=<%= time1 %>&time2=<%= time2 %>&emp_id=<%= emp_id %>&start=<%= startRow %>&rep_id=<%= report_id %>&lang=<%= lang %>" frameborder="0" scrolling="yes"></iframe>
									</div>
								</div>
