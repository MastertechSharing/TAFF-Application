...
<% 
	...
	session.setAttribute("action", "file_upload_message.jsp?&");
...
<%	
	...
	//	String lastName = fileName.substring(fileName.indexOf(".") +1);
		...
		if((fileName != "") && (lastName.equals("TXT") ||lastName.equals("txt") || lastName.equals("CSV") || lastName.equals("csv"))){
			try{
				rc.WriteDataLogFile("["+ses_user+"] Upload File Message [dbemployee]");
				 ...
				 */	data_arr = (message+",#").split(",");
					...
					if(!ckCode || data_arr.length != 4){
						msgerr.add(message+" --> ("+lb_format_file_err+")");
						no_pass++;
					}else if(data_arr[0].length() > 16){
						msgerr.add(message+" --> ("+alert_idcard_over16+")");
						no_pass++;
					}else if(data_arr[1].length() > 40){
						msgerr.add(message+" --> ("+alert_message_over40+")");
						no_pass++;
					}else if((data_arr[2].length() != 10)){
						msgerr.add(message+" --> ("+alertDT_upload+")" );
						no_pass++;
					}else if(!chkPatternDate(data_arr[2])){
						msgerr.add(message+" --> ("+alertDT_upload+")");
						no_pass++;
					}else if(Integer.parseInt(YMDToYYYYMMDD(dateToYMD(data_arr[2]))) < Integer.parseInt(YMDToYYYYMMDD(getCurrentDateyyyyMMdd()))){
						msgerr.add(message+" --> ("+alert_error_message_date+")" );
						no_pass++;
					}else{
						
						sql = "UPDATE dbemployee SET message='"+data_arr[1]+"', message_date='"+dateToYMD(data_arr[2])+"' WHERE (idcard = '"+data_arr[0]+"')";
						try{
							resultQry = stmtUp.executeUpdate(sql);
							if(resultQry != 0){
								rc.WriteDataLogFile("[" + ses_user + "] Add message idcard : " + data_arr[0] + " [dbemployee]");
								pass++;
							}else{	//	No Employee
								msgerr.add(message+" --> ("+alert_no_employee+" : "+data_arr[0]+")" );
								no_pass++;
							}
...
%>		
		<div class="body-display">
			<div class="container" style="margin-top: -20px;">
				...
				<div class="col-md-10">
					<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="file_upload_message.jsp">
						<div class="panel panel-primary"> 
							<div class="panel-heading">
								<h3 class="panel-title"> <b> <%= lb_message %> </b> </h3> 
							</div> 
								...
								<div class="table-responsive" style="border: 0px !important;" border="0">
									<div style="min-width: 800px; max-width: 100%; margin-left: 15px; margin-right: 15px;" border="0">
										...
										<div class="row">
											...
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_message_detail %> ( 40 <%= lb_letter %> ), </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
											<label class="col-md-10 col-xs-10" style="margin-top: 3px;"> <%= lb_message_date %> ( dd/mm/yyyy ) </label> 
										</div>
										<div class="row">
											<div class="col-md-2 col-xs-2"> &nbsp; </div>
